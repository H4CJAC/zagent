...
_ensure_bash() {
  ...
  if _have_cmd apk; then _run_privileged apk add --no-cache bash
    ...
    if _is_container_runtime; then
      _PACMAN_CFG="$(mktemp /tmp/cclawcore-pacman.XXXXXX.conf)"
...
usage() {
  cat <<'USAGE'
CclawCore installer — one-click bootstrap

Usage:
  ./install.sh [options]

The installer builds CclawCore, configures your provider and API key,
...
Examples:
  ...
  curl -fsSL https://cclawcorelabs.ai/install.sh | bash
...
Environment:
  CCLAWCORE_CONTAINER_CLI     Container CLI command (default: docker; auto-fallback: podman)
  CCLAWCORE_DOCKER_DATA_DIR   Host path for Docker config/workspace persistence
  CCLAWCORE_DOCKER_IMAGE      Docker image tag to build/run (default: cclawcore-bootstrap:local)
  CCLAWCORE_API_KEY           Used when --api-key is not provided
  CCLAWCORE_PROVIDER          Used when --provider is not provided (default: openrouter)
  CCLAWCORE_MODEL             Used when --model is not provided
  CCLAWCORE_CARGO_FEATURES    Extra cargo features for source builds (comma/space separated)
  CCLAWCORE_BOOTSTRAP_MIN_RAM_MB   Minimum RAM threshold for source build preflight (default: 2048)
  CCLAWCORE_BOOTSTRAP_MIN_DISK_MB  Minimum free disk threshold for source build preflight (default: 6144)
  CCLAWCORE_DISABLE_ALPINE_AUTO_DEPS
...
should_attempt_prebuilt_for_resources() {
  ...
  min_ram_mb="${CCLAWCORE_BOOTSTRAP_MIN_RAM_MB:-2048}"
  min_disk_mb="${CCLAWCORE_BOOTSTRAP_MIN_DISK_MB:-6144}"
...
resolve_asset_url() {
  local asset_name="$1"
  local api_url="https://api.github.com/repos/cclawcore-labs/cclawcore/releases"
...
install_prebuilt_binary() {
  ...
  asset_name="cclawcore-${target}.tar.gz"
  ...
  if [[ -z "$archive_url" ]]; then
    archive_url="https://github.com/cclawcore-labs/cclawcore/releases/latest/download/${asset_name}"
  fi

  temp_dir="$(mktemp -d -t cclawcore-prebuilt-XXXXXX)"
  ...
  extracted_bin="$temp_dir/cclawcore"
  if [[ ! -x "$extracted_bin" ]]; then
    extracted_bin="$(find "$temp_dir" -maxdepth 2 -type f -name cclawcore -perm -u+x | head -n 1 || true)"
  fi
  if [[ -z "$extracted_bin" || ! -x "$extracted_bin" ]]; then
    warn "Archive did not contain an executable cclawcore binary."
    rm -rf "$temp_dir"
    return 1
  fi
  ...
  mkdir -p "$install_dir"
  install -m 0755 "$extracted_bin" "$install_dir/cclawcore"
  rm -rf "$temp_dir"

  step_ok "Installed pre-built binary to $install_dir/cclawcore"
...
run_pacman() {
  ...
  local pacman_rc=0
  pacman_cfg_tmp="$(mktemp /tmp/cclawcore-pacman.XXXXXX.conf)"
...
MSG
      ...
      _xcode_test_file="$(mktemp /tmp/cclawcore-xcode-check.XXXXXX.c)"
...
ensure_default_config_and_workspace() {
  ...
  #
  # $1 — config directory  (e.g. ~/.cclawcore or $docker_data_dir/.cclawcore)
  # $2 — workspace directory (e.g. ~/.cclawcore/workspace or $docker_data_dir/workspace)
...
# CclawCore configuration — generated by install.sh
# Edit this file or run 'cclawcore onboard --tui' to reconfigure.
...
resolve_container_cli() {
  local requested_cli
  requested_cli="${CCLAWCORE_CONTAINER_CLI:-docker}"
  ...
  if [[ "$requested_cli" != "docker" ]]; then
    error "Set CCLAWCORE_CONTAINER_CLI to an installed Docker-compatible CLI (e.g., docker or podman)."
  else
    error "Install Docker, install podman, or set CCLAWCORE_CONTAINER_CLI to an available Docker-compatible CLI."
  fi
  exit 1
}
...
run_docker_bootstrap() {
  ...
  docker_image="${CCLAWCORE_DOCKER_IMAGE:-cclawcore-bootstrap:local}"
  fallback_image="ghcr.io/cclawcore-labs/cclawcore:latest"
  if [[ "$TEMP_CLONE" == true ]]; then
    default_data_dir="$HOME/.cclawcore-docker"
  else
    default_data_dir="$WORK_DIR/.cclawcore-docker"
  fi
  docker_data_dir="${CCLAWCORE_DOCKER_DATA_DIR:-$default_data_dir}"
  DOCKER_DATA_DIR="$docker_data_dir"

  mkdir -p "$docker_data_dir/.cclawcore" "$docker_data_dir/workspace"
  ...
  if [[ "$SKIP_BUILD" == false ]]; then
    ...
    if ! "$CONTAINER_CLI" image inspect "$docker_image" >/dev/null 2>&1; then
      ...
      info "Pulling official CclawCore image ($fallback_image)"
  ...
  config_mount="$docker_data_dir/.cclawcore:/cclawcore-data/.cclawcore"
  workspace_mount="$docker_data_dir/workspace:/cclawcore-data/workspace"
  ...
  if [[ ${#onboard_cmd[@]} -gt 0 ]]; then
    ...
    info "Docker image ready. Run cclawcore onboard --tui inside the container to configure."
  fi
...
ROOT_DIR="$SCRIPT_DIR"
REPO_URL="https://github.com/cclawcore-labs/cclawcore.git"
DOCKER_MODE=false
...
CONTAINER_CLI="${CCLAWCORE_CONTAINER_CLI:-docker}"
API_KEY="${CCLAWCORE_API_KEY:-}"
PROVIDER="${CCLAWCORE_PROVIDER:-openrouter}"
MODEL="${CCLAWCORE_MODEL:-}"
CARGO_FEATURES_INPUT="${CCLAWCORE_CARGO_FEATURES:-}"
...
if [[ "$DOCKER_MODE" == true ]]; then
  ...
  if [[ "$OS_NAME" == "Linux" && -z "${CCLAWCORE_DISABLE_ALPINE_AUTO_DEPS:-}" ]] && have_cmd apk; then
    find_missing_alpine_prereqs
    if [[ ${#ALPINE_MISSING_PKGS[@]} -gt 0 && "$INSTALL_SYSTEM_DEPS" == false ]]; then
      ...
      info "Auto-enabling system dependency installation (set CCLAWCORE_DISABLE_ALPINE_AUTO_DEPS=1 to disable)."
      INSTALL_SYSTEM_DEPS=true
    fi
  fi
...
if [[ ! -f "$WORK_DIR/Cargo.toml" ]]; then
  if [[ -f "$(pwd)/Cargo.toml" ]]; then
    ...
    TEMP_DIR="$(mktemp -d -t cclawcore-bootstrap-XXXXXX)"
...
echo
echo -e "  ${BOLD_BLUE}${CRAB} CclawCore Installer${RESET}"
...
EXISTING_VERSION=""
INSTALL_MODE="fresh"
if have_cmd cclawcore; then
  EXISTING_VERSION="$(cclawcore --version 2>/dev/null | awk '{print $NF}' || true)"
  INSTALL_MODE="upgrade"
elif [[ -x "$HOME/.cargo/bin/cclawcore" ]]; then
  EXISTING_VERSION="$("$HOME/.cargo/bin/cclawcore" --version 2>/dev/null | awk '{print $NF}' || true)"
  INSTALL_MODE="upgrade"
fi
...
if [[ "$INSTALL_MODE" == "upgrade" && -n "$EXISTING_VERSION" ]]; then
  step_dot "Existing CclawCore installation detected, upgrading from v${EXISTING_VERSION}"
elif [[ "$INSTALL_MODE" == "upgrade" ]]; then
  step_dot "Existing CclawCore installation detected, upgrading"
fi
...
if [[ "$DOCKER_MODE" == true ]]; then
  ...
  echo
  echo -e "${BOLD}Your containerized CclawCore data is persisted under:${RESET}"
  echo -e "  ${DIM}$DOCKER_DATA_DIR${RESET}"
  echo
  echo -e "${BOLD}Dashboard URL:${RESET} ${BLUE}http://127.0.0.1:42617${RESET}"
  echo
  echo -e "${BOLD}Next steps:${RESET}"
  echo -e "  ${DIM}cclawcore status${RESET}"
  echo -e "  ${DIM}cclawcore agent -m \"Hello, CclawCore!\"${RESET}"
  echo -e "  ${DIM}cclawcore gateway${RESET}"
  echo
  echo -e "${BOLD}Docs:${RESET} ${BLUE}https://www.cclawcorelabs.ai/docs${RESET}"
  exit 0
fi
...
echo
echo -e "${BOLD_BLUE}[2/3]${RESET} ${BOLD}Installing CclawCore${RESET}"
if [[ -n "$TARGET_VERSION" ]]; then
  step_dot "Installing CclawCore v${TARGET_VERSION}"
fi
if [[ "$SKIP_BUILD" == false ]]; then
...
if [[ "$SKIP_INSTALL" == false ]]; then
  step_dot "Installing cclawcore to cargo bin"

  # Clean up stale cargo install tracking from the old "cclawcore" package name
  # (renamed to "cclawcorelabs"). Without this, `cargo install cclawcorelabs` from
  # crates.io fails with "binary already exists as part of `cclawcore`".
  if have_cmd cargo; then
    if [[ -f "$HOME/.cargo/.crates.toml" ]] && grep -q '^"cclawcore ' "$HOME/.cargo/.crates.toml" 2>/dev/null; then
      step_dot "Removing stale cargo tracking for old 'cclawcore' package name"
      cargo uninstall cclawcore 2>/dev/null || true
    fi
  fi

  cargo install --path "$WORK_DIR" --force --locked "${CARGO_FEATURE_ARGS[@]}"
  step_ok "CclawCore installed"

  # Sync binary to ~/.local/bin so PATH lookups find the fresh version
  if [[ -d "$HOME/.local/bin" ]]; then
    cp -f "$HOME/.cargo/bin/cclawcore" "$HOME/.local/bin/cclawcore" 2>/dev/null && \
...
DESKTOP_DOWNLOAD_URL="https://www.cclawcorelabs.ai/download"
...
if [[ "$DEVICE_CLASS" == "desktop" ]]; then
  ...
  case "$OS_NAME" in
    Darwin)
      if [[ -d "/Applications/CclawCore.app" ]] || [[ -d "$HOME/Applications/CclawCore.app" ]]; then
        DESKTOP_APP_DETECTED=true
        step_ok "Companion app found (CclawCore.app)"
      fi
      ;;
    Linux)
      if have_cmd cclawcore-desktop; then
        DESKTOP_APP_DETECTED=true
        step_ok "Companion app found (cclawcore-desktop)"
      elif [[ -x "$HOME/.local/bin/cclawcore-desktop" ]]; then
        DESKTOP_APP_DETECTED=true
        step_ok "Companion app found (~/.local/bin/cclawcore-desktop)"
      fi
      ;;
  esac
  ...
  if [[ "$DESKTOP_APP_DETECTED" == false ]]; then
    ...
    echo -e "  Menu bar access to your CclawCore agent."
    ...
    echo -e "  ${DIM}Or run: cclawcore desktop --install${RESET}"
  fi
...
CCLAWCORE_BIN=""
if [[ -x "$HOME/.cargo/bin/cclawcore" ]]; then
  CCLAWCORE_BIN="$HOME/.cargo/bin/cclawcore"
elif [[ -x "$WORK_DIR/target/release/cclawcore" ]]; then
  CCLAWCORE_BIN="$WORK_DIR/target/release/cclawcore"
elif have_cmd cclawcore; then
  CCLAWCORE_BIN="cclawcore"
fi
...
if [[ "$SKIP_ONBOARD" == false && -n "$CCLAWCORE_BIN" ]]; then
  if [[ -n "$API_KEY" ]]; then
    # Non-interactive: apply provider/key directly
    step_dot "Configuring provider: ${PROVIDER}"
    ONBOARD_CMD=("$CCLAWCORE_BIN" onboard --api-key "$API_KEY" --provider "$PROVIDER")
    if [[ -n "$MODEL" ]]; then
      ONBOARD_CMD+=(--model "$MODEL")
    fi
    if "${ONBOARD_CMD[@]}" 2>/dev/null; then
      ...
      step_fail "Provider configuration failed — run cclawcore onboard --tui to retry"
    fi
    ...
    "$CCLAWCORE_BIN" onboard --tui || warn "TUI setup exited — run cclawcore onboard --tui to retry"
  else
    step_dot "No API key provided — run cclawcore onboard --tui to configure"
  fi
elif [[ "$SKIP_ONBOARD" == true ]]; then
  step_dot "Skipping configuration (run cclawcore onboard --tui later)"
elif [[ -z "$CCLAWCORE_BIN" ]]; then
  warn "CclawCore binary not found — cannot configure provider"
fi
...
_native_config_dir="${CCLAWCORE_CONFIG_DIR:-$HOME/.cclawcore}"
_native_workspace_dir="${CCLAWCORE_WORKSPACE:-$_native_config_dir/workspace}"
ensure_default_config_and_workspace "$_native_config_dir" "$_native_workspace_dir" "$PROVIDER"

# --- Gateway service management ---
if [[ -n "$CCLAWCORE_BIN" ]]; then
  # Try to install and start the gateway service
  step_dot "Checking gateway service"
  if "$CCLAWCORE_BIN" service install 2>/dev/null; then
    step_ok "Gateway service installed"
    if "$CCLAWCORE_BIN" service restart 2>/dev/null; then
      step_ok "Gateway service restarted"

    else
      step_fail "Gateway service restart failed — re-run with cclawcore service start"
    fi
  else
    step_dot "Gateway service not installed (run cclawcore service install later)"
  fi

  # --- Post-install doctor check ---
  step_dot "Running doctor to validate installation"
  if "$CCLAWCORE_BIN" doctor 2>/dev/null; then
    step_ok "Doctor complete"
  else
    warn "Doctor reported issues — run cclawcore doctor --fix to resolve"
  fi
fi
...
INSTALLED_VERSION=""
if [[ -n "$CCLAWCORE_BIN" ]]; then
  INSTALLED_VERSION="$("$CCLAWCORE_BIN" --version 2>/dev/null | awk '{print $NF}' || true)"
fi

# --- Success banner ---
echo
if [[ -n "$INSTALLED_VERSION" ]]; then
  echo -e "${BOLD_BLUE}${CRAB} CclawCore installed successfully (CclawCore ${INSTALLED_VERSION})!${RESET}"
else
  echo -e "${BOLD_BLUE}${CRAB} CclawCore installed successfully!${RESET}"
fi

if [[ -x "$HOME/.cargo/bin/cclawcore" ]] && ! have_cmd cclawcore; then
  echo
  warn "cclawcore is installed in $HOME/.cargo/bin, but that directory is not in PATH for this shell."
...
if [[ "$DEVICE_CLASS" == "desktop" ]]; then
  if [[ "$DESKTOP_APP_DETECTED" == true ]]; then
    echo -e "  ${DIM}cclawcore desktop${RESET}                ${DIM}# Launch the menu bar app${RESET}"
  else
    echo -e "  ${DIM}cclawcore desktop --install${RESET}      ${DIM}# Download the companion app${RESET}"
  fi
fi
echo
echo -e "${BOLD}Docs:${RESET} ${BLUE}https://www.cclawcorelabs.ai/docs${RESET}"
