...
fn main() {
    ...
    println!("cargo:rerun-if-changed=docs/assets/cclawcore-trans.png");
...
fn ensure_dashboard_assets(dist_dir: &Path) {
    ...
    let src = Path::new("docs/assets/cclawcore-trans.png");
    if !src.exists() {
        eprintln!(
            "cargo:warning=docs/assets/cclawcore-trans.png not found; skipping dashboard asset copy"
        );
        return;
    }

    let dst = dist_dir.join("cclawcore-trans.png");
    if let Err(e) = fs::copy(src, &dst) {
        eprintln!("cargo:warning=Failed to copy cclawcore-trans.png into web/dist/: {e}");
    }
}
