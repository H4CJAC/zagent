...
This directory contains everything needed to cross-compile CclawCore and deploy it to a Raspberry Pi over SSH.
...
| `rpi-config.toml` | Production config template deployed to `~/.cclawcore/config.toml` |
| `cclawcore.service` | systemd unit file installed on the Pi |
...
| `RPI_DIR` | `~/cclawcore` | Remote directory for the binary and `.env` |
...
2. **Stop service** — runs `sudo systemctl stop cclawcore` on the Pi (continues if not yet installed).
3. **Create remote directory** — ensures `$RPI_DIR` exists on the Pi.
4. **Copy binary** — SCPs the compiled binary to `$RPI_DIR/cclawcore`.
...
6. **Deploy config** — copies `rpi-config.toml` to `~/.cclawcore/config.toml`, preserving any `api_key` already present in the file.
7. **Install systemd service** — copies `cclawcore.service` to `/etc/systemd/system/`, then enables and restarts it.
...
sudo systemctl restart cclawcore
```
...
## Interacting with CclawCore on the Pi
...
```bash
ssh pi@raspberrypi.local 'journalctl -u cclawcore -f'
```
...
CclawCore is deployed with the `peripheral-rpi` feature, which enables two LLM-callable tools:
...
|------------|--------|-------------|
| `~/cclawcore/cclawcore` | compiled binary | Main agent binary |
| `~/cclawcore/.env` | created on first deploy | API key and environment variables |
| `~/.cclawcore/config.toml` | `rpi-config.toml` | Agent configuration |
| `/etc/systemd/system/cclawcore.service` | `cclawcore.service` | systemd service unit |
...
To customise, edit `~/.cclawcore/config.toml` directly on the Pi and restart the service.
...
```bash
ssh pi@raspberrypi.local 'sudo systemctl status cclawcore'
ssh pi@raspberrypi.local 'journalctl -u cclawcore -n 50 --no-pager'
```
...
```bash
ssh pi@raspberrypi.local 'file ~/cclawcore/cclawcore'
