#!/usr/bin/env bash
# deploy-rpi.sh — cross-compile CclawCore for Raspberry Pi and deploy via SSH.
#
...
#   RPI_DIR         — remote deployment dir           (default: /home/$RPI_USER/cclawcore)
...
RPI_DIR="${RPI_DIR:-/home/${RPI_USER}/cclawcore}"
TARGET="aarch64-unknown-linux-gnu"
FEATURES="hardware,peripheral-rpi"
BINARY="target/${TARGET}/release/cclawcore"
...
echo "==> Building CclawCore for Raspberry Pi (${TARGET})"
...
echo ""
echo "==> Stopping cclawcore service (if running)"
# shellcheck disable=SC2029
${SSH_CMD} ${SSH_OPTS} "${RPI_USER}@${RPI_HOST}" \
  "sudo systemctl stop cclawcore 2>/dev/null || true"
...
# ── 4. Deploy binary ──────────────────────────────────────────────────────────
echo ""
echo "==> Deploying binary to ${RPI_USER}@${RPI_HOST}:${RPI_DIR}/cclawcore"
${SCP_CMD} ${SCP_OPTS} "${BINARY}" "${RPI_USER}@${RPI_HOST}:${RPI_DIR}/cclawcore"
...
# ── 5. Deploy config ─────────────────────────────────────────────────────────
CONFIG_DEST="/home/${RPI_USER}/.cclawcore/config.toml"
echo ""
echo "==> Deploying config to ${CONFIG_DEST}"
# shellcheck disable=SC2029
${SSH_CMD} ${SSH_OPTS} "${RPI_USER}@${RPI_HOST}" "mkdir -p /home/${RPI_USER}/.cclawcore"
...
SERVICE_DEST="/etc/systemd/system/cclawcore.service"
echo ""
echo "==> Installing systemd service (requires sudo on the Pi)"
${SCP_CMD} ${SCP_OPTS} "scripts/cclawcore.service" "${RPI_USER}@${RPI_HOST}:/tmp/cclawcore.service"
# shellcheck disable=SC2029
${SSH_CMD} ${SSH_OPTS} "${RPI_USER}@${RPI_HOST}" \
  "sudo mv /tmp/cclawcore.service ${SERVICE_DEST} && \
   sudo systemctl daemon-reload && \
   sudo systemctl enable cclawcore && \
   sudo systemctl restart cclawcore && \
   sudo systemctl status cclawcore --no-pager || true"
...
# ── 8. Reset ACT LED trigger so CclawCore can control it ──────────────────────
echo ""
...
echo ""
echo "    CclawCore is running at http://${RPI_HOST}:8080"
echo "    POST /api/chat  — chat with the agent"
echo "    GET  /health    — health check"
echo ""
echo "    To check logs: ssh ${RPI_USER}@${RPI_HOST} 'journalctl -u cclawcore -f'"
