...
This guide covers native Nextcloud Talk integration for CclawCore.
...
Add this section in `~/.cclawcore/config.toml`:
...
allowed_users = ["*"]
# bot_name is the Nextcloud Talk display name of the bot (e.g. "cclawcore").
...
cclawcore daemon
# or
cclawcore gateway --host 127.0.0.1 --port 3000
```
...
When `webhook_secret` is configured, CclawCore verifies:
...
- CclawCore ignores bot-originated webhook events (`actorType = bots`).
- CclawCore ignores non-message/system events.
...
3. Confirm CclawCore receives and replies in the same room.
