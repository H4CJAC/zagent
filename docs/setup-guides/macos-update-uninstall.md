...
This page documents supported update and uninstall procedures for CclawCore on macOS (OS X).
...
- Homebrew: `/opt/homebrew/bin/cclawcore` (Apple Silicon) or `/usr/local/bin/cclawcore` (Intel)
- Cargo/bootstrap/manual: `~/.cargo/bin/cclawcore`
...
```bash
cclawcore service stop || true
cclawcore service uninstall || true
```

Service artifacts removed by `service uninstall`:

- `~/Library/LaunchAgents/com.cclawcore.daemon.plist`
...
```bash
brew uninstall cclawcore
```

Cargo/bootstrap/manual (`~/.cargo/bin/cclawcore`):

```bash
cargo uninstall cclawcore || true
rm -f ~/.cargo/bin/cclawcore
```
...
```bash
command -v cclawcore || echo "cclawcore binary not found"
pgrep -fl cclawcore || echo "No running cclawcore process"
```
