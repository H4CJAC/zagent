...
This page defines the fastest supported path to install and initialize CclawCore.
...
Default behavior is **app-only** (build/install CclawCore) and expects existing Rust toolchain.
...
```bash
curl -fsSL https://raw.githubusercontent.com/cclawcore-labs/cclawcore/master/install.sh | bash
```
...
This builds a local CclawCore image and launches onboarding inside a container while
persisting config/workspace to `./.cclawcore-docker`.

Container CLI defaults to `docker`. If Docker CLI is unavailable and `podman` exists,
the installer auto-falls back to `podman`. You can also set `CCLAWCORE_CONTAINER_CLI`
explicitly (for example: `CCLAWCORE_CONTAINER_CLI=podman ./install.sh --docker`).
...
Docker tag (`CCLAWCORE_DOCKER_IMAGE`, default: `cclawcore-bootstrap:local`); if missing,
it pulls `ghcr.io/cclawcore-labs/cclawcore:latest` and tags it locally before running.
...
are persisted in the data directory (default: `./.cclawcore-docker`, or `~/.cclawcore-docker`
when bootstrapping via `curl | bash`). You can override this path with `CCLAWCORE_DOCKER_DATA_DIR`.
...
The simplest way to run CclawCore long-term in Docker/Podman is with the provided
`docker-compose.yml` at the repository root. It uses a named volume (`cclawcore-data`)
...
If you installed via `./install.sh --docker` and want to reuse the `.cclawcore-docker`
...
podman run -d --name cclawcore \
  --restart unless-stopped \
  --userns keep-id \
  --user "$(id -u):$(id -g)" \
  -v "$PWD/.cclawcore-docker/.cclawcore:/cclawcore-data/.cclawcore:Z" \
  -v "$PWD/.cclawcore-docker/workspace:/cclawcore-data/workspace:Z" \
  -e HOME=/cclawcore-data \
  -e CCLAWCORE_WORKSPACE=/cclawcore-data/workspace \
  -p 42617:42617 \
  cclawcore-bootstrap:local \
...
# Remove the container (data in volumes/.cclawcore-docker is preserved)
docker rm cclawcore
...
saved in `.cclawcore-docker/.cclawcore/config.toml` and do not need to be passed again.
...
```bash
CCLAWCORE_API_KEY="sk-..." CCLAWCORE_PROVIDER="openrouter" ./install.sh
```
...
- `--install-system-deps`
- `--install-rust`
- `--skip-build` (in `--docker` mode: use local image if present, otherwise pull `ghcr.io/cclawcore-labs/cclawcore:latest`)
- `--skip-install`
- `--provider <id>`
