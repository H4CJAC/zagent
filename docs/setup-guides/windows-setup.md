...
This guide covers building and installing CclawCore on Windows.
...
```powershell
scoop bucket add cclawcore https://github.com/cclawcore-labs/scoop-cclawcore
scoop install cclawcore
```
...
copy target\x86_64-pc-windows-msvc\release\cclawcore.exe %USERPROFILE%\.cclawcore\bin\
```
...
CclawCore uses Cargo feature flags to control which integrations are compiled in:
...
2. **Initialize CclawCore:**
   ```cmd
   cclawcore init
   ```
3. **Configure your API key** in `%USERPROFILE%\.cclawcore\config.toml`
