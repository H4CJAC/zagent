...
This guide introduces multi-model concepts in CclawCore, including fallback provider chains, model-level fallbacks, and API key rotation for resilience.
...
When a provider experiences a transient error (timeout, connection failure, auth issue), CclawCore automatically attempts fallback providers in the order specified.

**Example**: If your primary provider is `openai` but it's temporarily unavailable, CclawCore can automatically fall back to `anthropic`, then `groq`.
...
When the primary provider recovers, CclawCore resumes using it (no sticky failover).
...
If `claude-opus-4-20250514` fails or is unavailable, CclawCore tries the fallback models in order while staying within the same provider (unless a provider-level fallback is also configured).

### API Key Rotation

For providers that frequently encounter rate limits, you can supply additional API keys that CclawCore will rotate through on `429` responses.
...
**Behavior**: If OpenAI times out or returns an error, CclawCore will retry twice with exponential backoff, then attempt the same request using Anthropic.
...
cclawcore doctor traces --contains "fallback"

# Show provider retry details
cclawcore doctor traces --contains "provider"

# Show rate-limit rotation
cclawcore doctor traces --contains "429"
```
...
3. Generic fallback: `CCLAWCORE_API_KEY`, then `API_KEY`
