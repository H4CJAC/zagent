...
CclawCore provides prebuilt binaries for Android devices.
...
The easiest way to run CclawCore on Android is via [Termux](https://termux.dev/).
...
# For 64-bit (aarch64):
curl -LO https://github.com/cclawcore-labs/cclawcore/releases/latest/download/cclawcore-aarch64-linux-android.tar.gz
tar xzf cclawcore-aarch64-linux-android.tar.gz

# For 32-bit (armv7):
curl -LO https://github.com/cclawcore-labs/cclawcore/releases/latest/download/cclawcore-armv7-linux-androideabi.tar.gz
tar xzf cclawcore-armv7-linux-androideabi.tar.gz
```
...
For advanced users who want to run CclawCore outside Termux:

```bash
# From your computer with ADB
adb push cclawcore /data/local/tmp/
adb shell chmod +x /data/local/tmp/cclawcore
adb shell /data/local/tmp/cclawcore --version
```
