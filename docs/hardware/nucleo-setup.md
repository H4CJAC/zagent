# CclawCore on Nucleo-F401RE — Step-by-Step Guide

Run CclawCore on your Mac or Linux host. Connect a Nucleo-F401RE via USB. Control GPIO (LED, pins) via Telegram or CLI.

---

## Get Board Info via Telegram (No Firmware Needed)

CclawCore can read chip info from the Nucleo over USB **without flashing any firmware**. Message your Telegram bot:
...
CclawCore includes everything for Nucleo-F401RE:
...
| Flash command | `cclawcore peripheral flash-nucleo` | Builds firmware, flashes via probe-rs |
...
### 1.2 Flash via CclawCore

From the cclawcore repo root:
...
| 5 | `cclawcore daemon` or `cclawcore agent -m "Turn on LED"` |
...
- **GPIO commands ignored** — Check `path` in config matches your serial port. Run `cclawcore peripheral list` to verify.
