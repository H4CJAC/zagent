...
<h1 align="center">🦀 CclawCore — Asistent AI Personal</h1>
...
> **Configurare recomandată:** rulează `cclawcore onboard` în terminalul tău. CclawCore Onboard te ghidează pas cu pas prin configurarea gateway-ului, workspace-ului, canalelor și provider-ului. Este calea de configurare recomandată și funcționează pe macOS, Linux și Windows (prin WSL2). Instalare nouă? Începe aici: [Începe](#pornire-rapidă)
...
`cclawcore onboard` rulează automat după instalare pentru a configura workspace-ul și provider-ul.
...
cclawcore gateway                # implicit: 127.0.0.1:42617
cclawcore gateway --port 0       # port aleatoriu (securitate îmbunătățită)
...
Actualizezi? Rulează `cclawcore doctor` după actualizare.
...
CclawCore poate importa workspace-ul, memoria și configurația OpenClaw:
...
Aceasta migrează intrările de memorie, fișierele workspace și configurația din `~/.openclaw/` în `~/.cclawcore/`. Configurația este convertită automat din JSON în TOML.

## Setări implicite de securitate (acces DM)

CclawCore se conectează la suprafețe de mesagerie reale. Tratează DM-urile primite ca intrare neîncredere.
...
- Aprobă cu: `cclawcore pairing approve <channel> <code>` (apoi expeditorul este adăugat pe o listă de permisiuni locală).
- DM-urile publice primite necesită un opt-in explicit în `config.toml`.
- Rulează `cclawcore doctor` pentru a identifica politici DM riscante sau configurate greșit.
...
| ---------- | ----------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-02-19 | _Critic_  | Nu suntem **afiliați** cu `openagen/cclawcore`, `cclawcore.org` sau `cclawcore.net`. Domeniile `cclawcore.org` și `cclawcore.net` indică în prezent fork-ul `openagen/cclawcore`, iar acel domeniu/depozit se dă drept site-ul/proiectul nostru oficial.                                                                                       | Nu aveți încredere în informații, binare, strângeri de fonduri sau anunțuri din acele surse. Folosiți doar [acest depozit](https://github.com/cclawcore-labs/cclawcore) și conturile noastre sociale verificate.                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
## Instantaneu Benchmark (CclawCore vs OpenClaw, Reproductibil)
...
CclawCore suportă profiluri de autentificare native abonament (multi-cont, criptate în repaus).

- Fișier de stocare: `~/.cclawcore/auth-profiles.json`
- Cheie de criptare: `~/.cclawcore/.secret_key`
...
Rădăcina workspace: `~/.cclawcore/workspace/` (configurabilă prin config).
...
Skill-uri: `~/.cclawcore/workspace/skills/<skill>/SKILL.md` sau `SKILL.toml`.
...
cclawcore onboard              # Asistent de configurare ghidată
cclawcore status               # Afișează starea daemon/agent
cclawcore doctor               # Rulează diagnostice de sistem

# Gateway + daemon
cclawcore gateway              # Pornește serverul gateway (127.0.0.1:42617)
cclawcore daemon               # Pornește runtime-ul autonom complet

# Agent
cclawcore agent                # Mod chat interactiv
cclawcore agent -m "message"   # Mod mesaj unic

# Gestionarea serviciilor
cclawcore service install      # Instalează ca serviciu OS (launchd/systemd)
cclawcore service start|stop|restart|status

# Canale
cclawcore channel list         # Listează canalele configurate
cclawcore channel doctor       # Verifică sănătatea canalelor
cclawcore channel bind-telegram 123456789

# Cron + programare
cclawcore cron list            # Listează sarcinile programate
cclawcore cron add "*/5 * * * *" --prompt "Check system health"
cclawcore cron remove <id>

# Memorie
cclawcore memory list          # Listează intrările de memorie
cclawcore memory get <key>     # Recuperează o memorie
cclawcore memory stats         # Statistici memorie
...
# Periferice hardware
cclawcore hardware discover    # Scanează dispozitivele conectate
cclawcore peripheral list      # Listează perifericele conectate
cclawcore peripheral flash     # Încarcă firmware pe dispozitiv
...
Sau sări peste pașii de mai sus și instalează totul (dependențe sistem, Rust, CclawCore) cu o singură comandă:
...
- Urmează [CclawCore Onboard](#pornire-rapidă) pentru configurare ghidată.
...
CclawCore a fost construit pentru smooth crab 🦀, un asistent AI rapid și eficient. Construit de Argenis De La Rosa și comunitate.
...
Dacă CclawCore te ajută în muncă și vrei să susții dezvoltarea continuă, poți dona aici:
...
Nou la CclawCore? Caută probleme etichetate [`good first issue`](https://github.com/cclawcore-labs/cclawcore/issues?q=is%3Aissue+is%3Aopen+label%3A%22good+first+issue%22) — vezi [Ghidul de Contribuție](CONTRIBUTING.md#first-time-contributors) pentru cum să începi. PR-urile create cu AI/vibe-coded sunt binevenite! 🤖
...
- `Skill` nou → `~/.cclawcore/workspace/skills/<name>/`
...
**Acesta este singurul depozit oficial CclawCore:**

> https://github.com/cclawcore-labs/cclawcore

Orice alt depozit, organizație, domeniu sau pachet care pretinde a fi „CclawCore" sau implică afiliere cu CclawCore Labs este **neautorizat și nu este afiliat cu acest proiect**. Fork-urile neautorizate cunoscute vor fi listate în [TRADEMARK.md](docs/maintainers/trademark.md).

Dacă întâmpini uzurpare de identitate sau utilizare abuzivă a mărcii comerciale, te rugăm [deschide o problemă](https://github.com/cclawcore-labs/cclawcore/issues).

---

## Licență

CclawCore este dual-licențiat pentru deschidere maximă și protecția contributorilor:
...
Numele și logo-ul **CclawCore** sunt mărci comerciale ale CclawCore Labs. Această licență nu acordă permisiunea de a le folosi pentru a implica aprobare sau afiliere. Vezi [TRADEMARK.md](docs/maintainers/trademark.md) pentru utilizări permise și interzise.
...
**CclawCore** — Zero overhead. Zero compromisuri. Implementează oriunde. Schimbă orice. 🦀
