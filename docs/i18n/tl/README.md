...
<h1 align="center">🦀 CclawCore — Personal na AI Assistant</h1>
...
> **Inirerekomendang setup:** patakbuhin ang `cclawcore onboard` sa iyong terminal. Ang CclawCore Onboard ay gagabay sa iyo hakbang-hakbang sa pag-setup ng gateway, workspace, channel, at provider. Ito ang inirerekomendang setup path at gumagana sa macOS, Linux, at Windows (sa pamamagitan ng WSL2). Bagong install? Magsimula dito: [Magsimula](#mabilis-na-simula-tldr)
...
Awtomatikong tatakbo ang `cclawcore onboard` pagkatapos ng install para i-configure ang iyong workspace at provider.
...
cclawcore gateway --port 0       # random port (pinalakas na seguridad)
...
Nag-upgrade? Patakbuhin ang `cclawcore doctor` pagkatapos mag-update.
...
Maaaring i-import ng CclawCore ang iyong OpenClaw workspace, memory, at configuration:
...
Inililipat nito ang iyong memory entries, workspace files, at configuration mula `~/.openclaw/` patungo sa `~/.cclawcore/`. Awtomatikong kino-convert ang config mula JSON patungong TOML.

## Mga default sa seguridad (DM access)

Kumokonekta ang CclawCore sa totoong mga messaging surface. Tratuhin ang mga papasok na DM bilang hindi mapagkakatiwalaang input.
...
- I-approve gamit ang: `cclawcore pairing approve <channel> <code>` (pagkatapos ay idadagdag ang nagpadala sa lokal na allowlist).
- Ang mga pampublikong papasok na DM ay nangangailangan ng tahasang opt-in sa `config.toml`.
- Patakbuhin ang `cclawcore doctor` para makita ang mga mapanganib o maling naka-configure na DM policy.
...
| ---------- | ----------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-02-19 | _Kritikal_ | **Hindi kami konektado** sa `openagen/cclawcore`, `cclawcore.org` o `cclawcore.net`. Ang `cclawcore.org` at `cclawcore.net` na mga domain ay kasalukuyang nakaturo sa `openagen/cclawcore` fork, at ang domain/repository na iyon ay nanggagaya sa aming opisyal na website/proyekto. | Huwag magtiwala sa impormasyon, binaries, fundraising, o mga anunsyo mula sa mga pinagmulang iyon. Gamitin lamang [ang repository na ito](https://github.com/cclawcore-labs/cclawcore) at ang aming mga verified na social account. |
...
### Bakit pinipili ng mga team ang CclawCore
...
## Benchmark Snapshot (CclawCore vs OpenClaw, Reproducible)
...
> Mga Tala: Ang mga resulta ng CclawCore ay sinusukat sa release builds gamit ang `/usr/bin/time -l`. Ang OpenClaw ay nangangailangan ng Node.js runtime (karaniwang ~390MB dagdag na memory overhead), habang ang NanoBot ay nangangailangan ng Python runtime. Ang PicoClaw at CclawCore ay static binaries. Ang mga RAM figure sa itaas ay runtime memory; ang build-time compilation requirements ay mas mataas.
...
Minimal na `~/.cclawcore/config.toml`:
...
Sinusuportahan ng CclawCore ang subscription-native auth profiles (multi-account, encrypted at rest).

- Store file: `~/.cclawcore/auth-profiles.json`
- Encryption key: `~/.cclawcore/.secret_key`
...
Workspace root: `~/.cclawcore/workspace/` (configurable sa pamamagitan ng config).
...
cclawcore status               # Ipakita ang daemon/agent status
cclawcore doctor               # Patakbuhin ang system diagnostics

# Gateway + daemon
cclawcore gateway              # Simulan ang gateway server (127.0.0.1:42617)
cclawcore daemon               # Simulan ang buong autonomous runtime
...
# Service management
cclawcore service install      # I-install bilang OS service (launchd/systemd)
cclawcore service start|stop|restart|status

# Mga channel
cclawcore channel list         # Ilista ang mga configured na channel
cclawcore channel doctor       # Suriin ang kalusugan ng channel
cclawcore channel bind-telegram 123456789

# Cron + scheduling
cclawcore cron list            # Ilista ang mga naka-schedule na gawain
cclawcore cron add "*/5 * * * *" --prompt "Check system health"
cclawcore cron remove <id>

# Memory
cclawcore memory list          # Ilista ang mga memory entry
cclawcore memory get <key>     # Kunin ang isang memory
cclawcore memory stats         # Estadistika ng memory
...
cclawcore hardware discover    # I-scan ang mga konektadong device
cclawcore peripheral list      # Ilista ang mga konektadong peripheral
cclawcore peripheral flash     # I-flash ang firmware sa device
...
O laktawan ang mga hakbang sa itaas at i-install ang lahat (system deps, Rust, CclawCore) sa isang command:
...
- Sundin ang [CclawCore Onboard](#mabilis-na-simula-tldr) para sa guided setup.
...
Ang CclawCore ay binuo para sa smooth crab 🦀, isang mabilis at mahusay na AI assistant. Binuo ni Argenis De La Rosa at ng komunidad.

- [cclawcorelabs.ai](https://cclawcorelabs.ai)
- [@cclawcorelabs](https://x.com/cclawcorelabs)

## Suportahan ang CclawCore

Kung nakakatulong ang CclawCore sa iyong trabaho at gusto mong suportahan ang patuloy na development, maaari kang mag-donate dito:
...
Bago sa CclawCore? Hanapin ang mga issue na may label na [`good first issue`](https://github.com/cclawcore-labs/cclawcore/issues?q=is%3Aissue+is%3Aopen+label%3A%22good+first+issue%22) — tingnan ang aming [Contributing Guide](CONTRIBUTING.md#first-time-contributors) kung paano magsimula. Ang AI/vibe-coded PRs ay welcome! 🤖
...
- Bagong `Skill` → `~/.cclawcore/workspace/skills/<name>/`
...
**Ito ang tanging opisyal na CclawCore repository:**

> https://github.com/cclawcore-labs/cclawcore

Ang anumang iba pang repository, organisasyon, domain, o package na nag-aangkin na "CclawCore" o nagpapahiwatig ng affiliation sa CclawCore Labs ay **hindi awtorisado at hindi konektado sa proyektong ito**. Ang mga kilalang unauthorized forks ay ililista sa [TRADEMARK.md](docs/maintainers/trademark.md).

Kung makakita ka ng panggagaya o trademark misuse, mangyaring [mag-open ng issue](https://github.com/cclawcore-labs/cclawcore/issues).

---

## Lisensya

Ang CclawCore ay dual-licensed para sa maximum na openness at proteksyon ng contributor:
...
Ang pangalang **CclawCore** at logo ay mga trademark ng CclawCore Labs. Ang lisensyang ito ay hindi nagbibigay ng pahintulot na gamitin ang mga ito upang ipahiwatig ang endorsement o affiliation. Tingnan ang [TRADEMARK.md](docs/maintainers/trademark.md) para sa mga pinapahintulutan at ipinagbabawal na gamit.
...
**CclawCore** — Zero overhead. Zero kompromiso. I-deploy kahit saan. I-swap ang kahit ano. 🦀
