...
<h1 align="center">🦀 CclawCore — Προσωπικός Βοηθός Τεχνητής Νοημοσύνης</h1>
...
Το `cclawcore onboard` εκτελείται αυτόματα μετά την εγκατάσταση για τη ρύθμιση του χώρου εργασίας και του παρόχου.
...
cclawcore gateway                # προεπιλογή: 127.0.0.1:42617
cclawcore gateway --port 0       # τυχαία θύρα (ενισχυμένη ασφάλεια)
...
Αναβάθμιση; Εκτελέστε `cclawcore doctor` μετά την ενημέρωση.
...
Το CclawCore μπορεί να εισάγει τον χώρο εργασίας, τη μνήμη και τη ρύθμιση παραμέτρων του OpenClaw:
...
Αυτό μετεγκαθιστά τις εγγραφές μνήμης, τα αρχεία χώρου εργασίας και τη ρύθμιση παραμέτρων από `~/.openclaw/` σε `~/.cclawcore/`. Η ρύθμιση μετατρέπεται αυτόματα από JSON σε TOML.

## Προεπιλογές ασφάλειας (πρόσβαση DM)

Το CclawCore συνδέεται σε πραγματικές επιφάνειες μηνυμάτων. Αντιμετωπίστε τα εισερχόμενα DM ως μη αξιόπιστη είσοδο.
...
- Εγκρίνετε με: `cclawcore pairing approve <channel> <code>` (τότε ο αποστολέας προστίθεται σε τοπική λίστα επιτρεπόμενων).
- Τα δημόσια εισερχόμενα DM απαιτούν ρητή ενεργοποίηση στο `config.toml`.
- Εκτελέστε `cclawcore doctor` για να εντοπίσετε επικίνδυνες ή εσφαλμένες πολιτικές DM.
...
| ---------- | ----------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-02-19 | _Κρίσιμο_ | **Δεν** είμαστε συνδεδεμένοι με `openagen/cclawcore`, `cclawcore.org` ή `cclawcore.net`. Οι τομείς `cclawcore.org` και `cclawcore.net` δείχνουν αυτή τη στιγμή στο fork `openagen/cclawcore`, και αυτός ο τομέας/αποθετήριο υποδύονται τον επίσημο ιστότοπο/έργο μας. | Μην εμπιστεύεστε πληροφορίες, δυαδικά αρχεία, εκστρατείες χρηματοδότησης ή ανακοινώσεις από αυτές τις πηγές. Χρησιμοποιήστε μόνο [αυτό το αποθετήριο](https://github.com/cclawcore-labs/cclawcore) και τους επαληθευμένους λογαριασμούς μας στα μέσα κοινωνικής δικτύωσης. |
...
### Γιατί οι ομάδες επιλέγουν το CclawCore
...
## Στιγμιότυπο Benchmark (CclawCore vs OpenClaw, Αναπαραγώγιμο)
...
> Σημειώσεις: Τα αποτελέσματα του CclawCore μετρήθηκαν σε release builds χρησιμοποιώντας `/usr/bin/time -l`. Το OpenClaw απαιτεί Node.js runtime (τυπικά ~390MB επιπλέον επιβάρυνση μνήμης), ενώ το NanoBot απαιτεί Python runtime. Τα PicoClaw και CclawCore είναι στατικά δυαδικά. Τα στοιχεία RAM παραπάνω αφορούν μνήμη χρόνου εκτέλεσης· οι απαιτήσεις μεταγλώττισης κατά τον χρόνο κατασκευής είναι υψηλότερες.
...
Ελάχιστο `~/.cclawcore/config.toml`:
...
Το CclawCore υποστηρίζει native προφίλ πιστοποίησης συνδρομής (πολλαπλοί λογαριασμοί, κρυπτογραφημένα σε αδράνεια).

- Αρχείο αποθήκευσης: `~/.cclawcore/auth-profiles.json`
- Κλειδί κρυπτογράφησης: `~/.cclawcore/.secret_key`
...
Ρίζα χώρου εργασίας: `~/.cclawcore/workspace/` (ρυθμιζόμενο μέσω config).
...
Δεξιότητες: `~/.cclawcore/workspace/skills/<skill>/SKILL.md` ή `SKILL.toml`.
...
cclawcore onboard              # Οδηγός καθοδηγούμενης ρύθμισης
cclawcore status               # Εμφάνιση κατάστασης daemon/agent
cclawcore doctor               # Εκτέλεση διαγνωστικών συστήματος

# Gateway + daemon
cclawcore gateway              # Εκκίνηση gateway server (127.0.0.1:42617)
cclawcore daemon               # Εκκίνηση πλήρους αυτόνομου χρόνου εκτέλεσης

# Πράκτορας
cclawcore agent                # Διαδραστική λειτουργία συνομιλίας
cclawcore agent -m "message"   # Λειτουργία μεμονωμένου μηνύματος

# Διαχείριση υπηρεσίας
cclawcore service install      # Εγκατάσταση ως υπηρεσία OS (launchd/systemd)
cclawcore service start|stop|restart|status

# Κανάλια
cclawcore channel list         # Λίστα ρυθμισμένων καναλιών
cclawcore channel doctor       # Έλεγχος υγείας καναλιών
cclawcore channel bind-telegram 123456789

# Cron + προγραμματισμός
cclawcore cron list            # Λίστα προγραμματισμένων εργασιών
cclawcore cron add "*/5 * * * *" --prompt "Check system health"
cclawcore cron remove <id>

# Μνήμη
cclawcore memory list          # Λίστα εγγραφών μνήμης
cclawcore memory get <key>     # Ανάκτηση μνήμης
cclawcore memory stats         # Στατιστικά μνήμης
...
# Περιφερειακά υλικού
cclawcore hardware discover    # Σάρωση για συνδεδεμένες συσκευές
cclawcore peripheral list      # Λίστα συνδεδεμένων περιφερειακών
cclawcore peripheral flash     # Flash firmware σε συσκευή
...
Ή παραλείψτε τα παραπάνω βήματα και εγκαταστήστε τα πάντα (εξαρτήσεις συστήματος, Rust, CclawCore) με μία εντολή:
...
- Ακολουθήστε [CclawCore Onboard](#γρήγορη-εκκίνηση-tldr) για καθοδηγούμενη ρύθμιση.
...
Το CclawCore δημιουργήθηκε για τον smooth crab 🦀, έναν γρήγορο και αποδοτικό βοηθό AI. Δημιουργήθηκε από τον Argenis De La Rosa και την κοινότητα.

- [cclawcorelabs.ai](https://cclawcorelabs.ai)
- [@cclawcorelabs](https://x.com/cclawcorelabs)

## Υποστήριξη CclawCore

Αν το CclawCore βοηθά τη δουλειά σας και θέλετε να υποστηρίξετε τη συνεχιζόμενη ανάπτυξη, μπορείτε να κάνετε δωρεά εδώ:
...
Νέος στο CclawCore; Ψάξτε για issues με ετικέτα [`good first issue`](https://github.com/cclawcore-labs/cclawcore/issues?q=is%3Aissue+is%3Aopen+label%3A%22good+first+issue%22) — δείτε τον [Οδηγό Συνεισφοράς](CONTRIBUTING.md#first-time-contributors) για το πώς να ξεκινήσετε. PR με AI/vibe-coding καλοδεχούμενα! 🤖
...
- Νέο `Skill` → `~/.cclawcore/workspace/skills/<name>/`
...
**Αυτό είναι το μόνο επίσημο αποθετήριο CclawCore:**

> https://github.com/cclawcore-labs/cclawcore

Οποιοδήποτε άλλο αποθετήριο, οργανισμός, τομέας ή πακέτο που ισχυρίζεται ότι είναι "CclawCore" ή υπονοεί σχέση με τα CclawCore Labs είναι **μη εξουσιοδοτημένο και δεν σχετίζεται με αυτό το έργο**. Τα γνωστά μη εξουσιοδοτημένα forks θα αναφέρονται στο [TRADEMARK.md](docs/maintainers/trademark.md).

Αν αντιμετωπίσετε πλαστοπροσωπία ή κατάχρηση εμπορικού σήματος, παρακαλούμε [ανοίξτε ένα issue](https://github.com/cclawcore-labs/cclawcore/issues).

---

## Άδεια

Το CclawCore έχει διπλή άδεια για μέγιστη ανοιχτότητα και προστασία συνεισφερόντων:
...
Το όνομα **CclawCore** και το λογότυπο είναι εμπορικά σήματα της CclawCore Labs. Αυτή η άδεια δεν παρέχει δικαίωμα χρήσης τους για να υπονοήσετε υποστήριξη ή σχέση. Δείτε [TRADEMARK.md](docs/maintainers/trademark.md) για επιτρεπόμενες και απαγορευμένες χρήσεις.
...
**CclawCore** — Μηδενική επιβάρυνση. Μηδενικοί συμβιβασμοί. Ανάπτυξη οπουδήποτε. Εναλλαγή οτιδήποτε. 🦀
