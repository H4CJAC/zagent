...
本指南介绍 CclawCore 的原生 Nextcloud Talk 集成。
...
在 `~/.cclawcore/config.toml` 中添加以下部分：
...
- 设置 `CCLAWCORE_NEXTCLOUD_TALK_WEBHOOK_SECRET` 时会覆盖 `webhook_secret`。
...
配置 `webhook_secret` 时，CclawCore 会验证：
...
- CclawCore 忽略来自机器人的 webhook 事件（`actorType = bots`）。
- CclawCore 忽略非消息/系统事件。
