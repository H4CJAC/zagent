...
- 如果不可避免需要类似身份的上下文，仅使用 CclawCore 范围的角色/标签（例如 `CclawCoreAgent`、`CclawCoreOperator`、`cclawcore_user`）。
- 推荐的身份安全命名调色板：
    - 参与者标签：`CclawCoreAgent`、`CclawCoreOperator`、`CclawCoreMaintainer`、`cclawcore_user`
    - 服务/运行时标签：`cclawcore_bot`、`cclawcore_service`、`cclawcore_runtime`、`cclawcore_node`
    - 环境标签：`cclawcore_project`、`cclawcore_workspace`、`cclawcore_channel`
