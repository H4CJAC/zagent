# 添加开发板和工具 — CclawCore 硬件指南

本指南解释如何向 CclawCore 添加新的硬件开发板和自定义工具。

## 快速开始：通过 CLI 添加开发板

```bash
# 添加开发板（更新 ~/.cclawcore/config.toml）
cclawcore peripheral add nucleo-f401re /dev/ttyACM0
cclawcore peripheral add arduino-uno /dev/cu.usbmodem12345
cclawcore peripheral add rpi-gpio native   # 用于树莓派 GPIO（Linux）
...
使用 `rag-pdf` 特性时，CclawCore 可以索引 PDF 文件：
...
2. **添加到配置** — `cclawcore peripheral add my-board /dev/ttyUSB0`
...
| 命令 | 描述 |
|---------|-------------|
| `cclawcore peripheral list` | 列出已配置的开发板 |
| `cclawcore peripheral add <board> <path>` | 添加开发板（写入配置） |
| `cclawcore peripheral flash` | 烧录 Arduino 固件 |
| `cclawcore peripheral flash-nucleo` | 烧录 Nucleo 固件 |
| `cclawcore hardware discover` | 列出 USB 设备 |
| `cclawcore hardware info` | 通过 probe-rs 获取芯片信息 |
