...
1. `CCLAWCORE_WORKSPACE` 覆盖（如果设置）
2. 持久化的 `~/.cclawcore/active_workspace.toml` 标记（如果存在）
3. 默认 `~/.cclawcore/config.toml`

CclawCore 在启动时以 `INFO` 级别记录解析后的配置：

- `Config loaded` 包含字段：`path`、`workspace`、`source`、`initialized`

模式导出命令：

- `cclawcore config schema`（将 JSON Schema 草案 2020-12 打印到 stdout）
...
| `otel_service_name` | `cclawcore` | 发送到 OTLP 收集器的服务名称 |
...
1. `CCLAWCORE_PROVIDER`（显式覆盖，非空时始终优先）
...
- 当你有意让运行时环境覆盖非默认配置的提供商时，请使用 `CCLAWCORE_PROVIDER`。
...
- 当 `enabled = true` 且不存在 OTP 密钥时，CclawCore 会生成一个并打印一次注册 URI。
...
| `state_file` | `~/.cclawcore/estop-state.json` | 持久化 estop 状态路径 |
...
- 使用 CLI 命令 `cclawcore estop` 启动，`cclawcore estop resume` 清除级别。
...
- 安全优先默认：除非 `open_skills_enabled = true`，否则 CclawCore **不会**克隆或同步 `open-skills`。
- 环境覆盖：
  - `CCLAWCORE_OPEN_SKILLS_ENABLED` 接受 `1/0`、`true/false`、`yes/no`、`on/off`。
  - `CCLAWCORE_OPEN_SKILLS_DIR` 非空时覆盖仓库路径。
  - `CCLAWCORE_SKILLS_PROMPT_MODE` 接受 `full` 或 `compact`。
- 启用标志的优先级：`CCLAWCORE_OPEN_SKILLS_ENABLED` → `config.toml` 中的 `skills.open_skills_enabled` → 默认 `false`。
- 建议在低上下文本地模型上使用 `prompt_injection_mode = \"compact\"`，以减少启动提示大小，同时按需保留技能文件可用。
- 技能加载和 `cclawcore skills install` 都会应用静态安全审计。包含符号链接、类脚本文件、高风险 shell  payload 片段或不安全 markdown 链接遍历的技能会被拒绝。
...
- CclawCore 请求 Composio v3 工具时使用 `toolkit_versions=latest`，并使用 `version=\"latest\"` 执行工具，以避免过时的默认工具版本。
...
3. 在重启/部署前使用 `cclawcore doctor` 验证。
...
- 当 `cclawcore channel start` 运行时，`default_provider`、`default_model`、`default_temperature`、`api_key`、`api_url` 和 `reliability.*` 的更新会在下一条入站消息时从 `config.toml` 热应用。
...
- 设置时 `CCLAWCORE_LINQ_SIGNING_SECRET` 覆盖 `signing_secret`。
...
- 设置时 `CCLAWCORE_NEXTCLOUD_TALK_WEBHOOK_SECRET` 覆盖 `webhook_secret`。
