...
6. **运行时状态陈旧**：配置已更改但 `cclawcore daemon` 未重启。

---

## 1. 配置命名空间

所有渠道设置都位于 `~/.cclawcore/config.toml` 的 `channels_config` 下。
...
运行 `cclawcore channel start`（或守护进程模式）时，Telegram 和 Discord 现在支持发送者范围的运行时切换：
...
- 模型缓存预览来自 `cclawcore models refresh --provider <ID>`。
- 这些是运行时聊天命令，不是 CLI 子命令。

## 入站图像标记协议

CclawCore 通过内联消息标记支持多模态输入：
...
如果存在 `[channels_config.matrix]`、`[channels_config.lark]` 或 `[channels_config.feishu]`，但对应的功能未编译进去，`cclawcore channel list`、`cclawcore channel doctor` 和 `cclawcore channel start` 会报告该渠道在此构建中被故意跳过。
...
access_token = \"syt_...\"
user_id = \"@cclawcore:matrix.example.com\"   # 可选，推荐用于 E2EE
...
- `CCLAWCORE_NEXTCLOUD_TALK_WEBHOOK_SECRET` 会覆盖配置中的密钥。
...
- `CCLAWCORE_LINQ_SIGNING_SECRET` 会覆盖配置中的密钥。
