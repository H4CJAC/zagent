...
<h1 align="center">🦀 CclawCore — Kişisel AI Asistanı</h1>
...
> **Önerilen kurulum:** terminalinizde `cclawcore onboard` komutunu çalıştırın. CclawCore Onboard, gateway, workspace, kanallar ve sağlayıcı kurulumunda sizi adım adım yönlendirir. Önerilen kurulum yoludur ve macOS, Linux ve Windows'ta (WSL2 ile) çalışır. Yeni kurulum mu? Buradan başlayın: [Başlarken](#hızlı-başlangıç)
...
`cclawcore onboard` kurulumdan sonra workspace ve sağlayıcınızı yapılandırmak için otomatik olarak çalışır.
...
cclawcore gateway                # varsayılan: 127.0.0.1:42617
cclawcore gateway --port 0       # rastgele port (güvenlik güçlendirilmiş)
...
Güncelleme mi yapıyorsunuz? Güncellemeden sonra `cclawcore doctor` çalıştırın.
...
CclawCore, OpenClaw workspace'inizi, belleğinizi ve yapılandırmanızı içe aktarabilir:
...
Bu, bellek girişlerinizi, workspace dosyalarınızı ve yapılandırmanızı `~/.openclaw/` dizininden `~/.cclawcore/` dizinine taşır. Yapılandırma otomatik olarak JSON'dan TOML'a dönüştürülür.

## Güvenlik varsayılanları (DM erişimi)

CclawCore gerçek mesajlaşma platformlarına bağlanır. Gelen DM'leri güvenilmeyen girdi olarak değerlendirin.
...
- Şununla onaylayın: `cclawcore pairing approve <channel> <code>` (ardından gönderen yerel izin listesine eklenir).
- Genel gelen DM'ler, `config.toml`'da açık bir opt-in gerektirir.
- Riskli veya yanlış yapılandırılmış DM politikalarını tespit etmek için `cclawcore doctor` çalıştırın.
...
| ---------- | ----------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-02-19 | _Kritik_  | `openagen/cclawcore`, `cclawcore.org` veya `cclawcore.net` ile **bağlantılı değiliz**. `cclawcore.org` ve `cclawcore.net` alan adları şu anda `openagen/cclawcore` fork'una yönlendirmektedir ve bu alan adı/depo, resmi web sitemizi/projemizi taklit etmektedir.                                                                                       | Bu kaynaklardan gelen bilgilere, ikili dosyalara, bağış toplama faaliyetlerine veya duyurulara güvenmeyin. Yalnızca [bu depoyu](https://github.com/cclawcore-labs/cclawcore) ve doğrulanmış sosyal hesaplarımızı kullanın.                                                                                                                                                                                                                                                                                                                                                                       |
...
### Ekipler neden CclawCore'u tercih ediyor
...
## Benchmark Özeti (CclawCore vs OpenClaw, Tekrarlanabilir)
...
> Notlar: CclawCore sonuçları, `/usr/bin/time -l` kullanılarak release derlemelerinde ölçülmüştür. OpenClaw, Node.js çalışma zamanı gerektirir (tipik olarak ~390MB ek bellek yükü), NanoBot ise Python çalışma zamanı gerektirir. PicoClaw ve CclawCore statik ikili dosyalardır. Yukarıdaki RAM rakamları çalışma zamanı belleğidir; derleme gereksinimleri daha yüksektir.
...
CclawCore, yerel abonelik yetkilendirme profillerini destekler (çoklu hesap, durağan halde şifreli).

- Depolama dosyası: `~/.cclawcore/auth-profiles.json`
- Şifreleme anahtarı: `~/.cclawcore/.secret_key`
...
Workspace kök dizini: `~/.cclawcore/workspace/` (config ile yapılandırılabilir).
...
Yetenekler: `~/.cclawcore/workspace/skills/<skill>/SKILL.md` veya `SKILL.toml`.
...
```bash
# Workspace yönetimi
cclawcore onboard              # Rehberli kurulum sihirbazı
cclawcore status               # Daemon/ajan durumunu göster
cclawcore doctor               # Sistem tanılaması çalıştır

# Gateway + daemon
cclawcore gateway              # Gateway sunucusunu başlat (127.0.0.1:42617)
cclawcore daemon               # Tam otonom çalışma zamanını başlat

# Ajan
cclawcore agent                # Etkileşimli sohbet modu
cclawcore agent -m "message"   # Tek mesaj modu

# Hizmet yönetimi
cclawcore service install      # OS hizmeti olarak yükle (launchd/systemd)
cclawcore service start|stop|restart|status

# Kanallar
cclawcore channel list         # Yapılandırılmış kanalları listele
cclawcore channel doctor       # Kanal sağlığını kontrol et
cclawcore channel bind-telegram 123456789

# Cron + zamanlama
cclawcore cron list            # Zamanlanmış görevleri listele
cclawcore cron add "*/5 * * * *" --prompt "Check system health"
cclawcore cron remove <id>

# Bellek
cclawcore memory list          # Bellek girişlerini listele
cclawcore memory get <key>     # Bir bellek al
cclawcore memory stats         # Bellek istatistikleri
...
cclawcore hardware discover    # Bağlı cihazları tara
cclawcore peripheral list      # Bağlı çevre birimlerini listele
cclawcore peripheral flash     # Cihaza firmware yükle
...
Veya yukarıdaki adımları atlayın ve her şeyi (sistem bağımlılıkları, Rust, CclawCore) tek komutla yükleyin:
...
- Rehberli kurulum için [CclawCore Onboard](#hızlı-başlangıç) takip edin.
...
CclawCore, smooth crab 🦀 için inşa edildi — hızlı ve verimli bir AI asistanı. Argenis De La Rosa ve topluluk tarafından geliştirildi.

- [cclawcorelabs.ai](https://cclawcorelabs.ai)
- [@cclawcorelabs](https://x.com/cclawcorelabs)

## CclawCore'u Destekleyin

CclawCore işinize yarıyorsa ve süregelen geliştirmeyi desteklemek istiyorsanız, buradan bağış yapabilirsiniz:
...
CclawCore'da yeni misiniz? [`good first issue`](https://github.com/cclawcore-labs/cclawcore/issues?q=is%3Aissue+is%3Aopen+label%3A%22good+first+issue%22) etiketli issue'ları arayın — nasıl başlayacağınızı öğrenmek için [Katkıda Bulunma Rehberi](CONTRIBUTING.md#first-time-contributors)mize bakın. AI/vibe-coded PR'lar hoş geldiniz! 🤖
...
- Yeni `Skill` → `~/.cclawcore/workspace/skills/<name>/`
...
**Bu, tek resmi CclawCore deposudur:**

> https://github.com/cclawcore-labs/cclawcore

"CclawCore" olduğunu iddia eden veya CclawCore Labs ile bağlantı ima eden başka herhangi bir depo, organizasyon, alan adı veya paket **yetkisiz olup bu projeyle bağlantılı değildir**. Bilinen yetkisiz fork'lar [TRADEMARK.md](docs/maintainers/trademark.md)'de listelenecektir.

Kimlik taklidi veya ticari marka kötüye kullanımıyla karşılaşırsanız, lütfen [bir issue açın](https://github.com/cclawcore-labs/cclawcore/issues).

---

## Lisans

CclawCore, maksimum açıklık ve katkıda bulunan koruması için çift lisanslıdır:
...
**CclawCore** adı ve logosu, CclawCore Labs'ın ticari markalarıdır. Bu lisans, onay veya bağlantı ima etmek için bunları kullanma izni vermez. İzin verilen ve yasaklanan kullanımlar için [TRADEMARK.md](docs/maintainers/trademark.md)'ye bakın.
...
**CclawCore** — Sıfır ek yük. Sıfır uzlaşma. Her yere dağıtın. Her şeyi değiştirin. 🦀
