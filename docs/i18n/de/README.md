...
<h1 align="center">🦀 CclawCore — Persönlicher KI-Assistent</h1>
...
> **Empfohlene Einrichtung:** Führe `cclawcore onboard` in deinem Terminal aus. CclawCore Onboard führt dich Schritt für Schritt durch die Einrichtung von Gateway, Workspace, Kanälen und Provider. Es ist der empfohlene Einrichtungspfad und funktioniert auf macOS, Linux und Windows (über WSL2). Neue Installation? Starte hier: [Erste Schritte](#schnellstart)
...
`cclawcore onboard` wird nach der Installation automatisch ausgeführt, um deinen Workspace und Provider zu konfigurieren.
...
cclawcore gateway --port 0       # Zufälliger Port (gehärtete Sicherheit)
...
Aktualisierung? Führe `cclawcore doctor` nach dem Update aus.
...
CclawCore kann deinen OpenClaw-Workspace, Speicher und Konfiguration importieren:
...
Dies migriert deine Speichereinträge, Workspace-Dateien und Konfiguration von `~/.openclaw/` nach `~/.cclawcore/`. Die Konfiguration wird automatisch von JSON nach TOML konvertiert.

## Sicherheitsstandards (DM-Zugriff)

CclawCore verbindet sich mit echten Messaging-Oberflächen. Behandle eingehende DMs als nicht vertrauenswürdige Eingabe.
...
- Genehmige mit: `cclawcore pairing approve <channel> <code>` (der Absender wird dann zu einer lokalen Allowlist hinzugefügt).
- Öffentliche eingehende DMs erfordern eine explizite Aktivierung in `config.toml`.
- Führe `cclawcore doctor` aus, um riskante oder falsch konfigurierte DM-Richtlinien aufzudecken.
...
| ---------- | ----------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-02-19 | _Kritisch_  | Wir sind **nicht verbunden** mit `openagen/cclawcore`, `cclawcore.org` oder `cclawcore.net`. Die Domains `cclawcore.org` und `cclawcore.net` verweisen derzeit auf den Fork `openagen/cclawcore`, und diese Domain/dieses Repository geben sich als unsere offizielle Website/unser offizielles Projekt aus.                                                                                       | Vertraue keinen Informationen, Binaries, Spendenaktionen oder Ankündigungen aus diesen Quellen. Verwende nur [dieses Repository](https://github.com/cclawcore-labs/cclawcore) und unsere verifizierten Social-Media-Konten.                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
## Benchmark-Übersicht (CclawCore vs OpenClaw, reproduzierbar)
...
CclawCore unterstützt native Abonnement-Authentifizierungsprofile (Multi-Account, verschlüsselt im Ruhezustand).

- Speicherdatei: `~/.cclawcore/auth-profiles.json`
- Verschlüsselungsschlüssel: `~/.cclawcore/.secret_key`
...
Workspace-Root: `~/.cclawcore/workspace/` (konfigurierbar über Config).
...
Skills: `~/.cclawcore/workspace/skills/<skill>/SKILL.md` oder `SKILL.toml`.
...
cclawcore onboard              # Geführter Einrichtungsassistent
cclawcore status               # Daemon/Agenten-Status anzeigen
cclawcore doctor               # Systemdiagnose ausführen

# Gateway + Daemon
cclawcore gateway              # Gateway-Server starten (127.0.0.1:42617)
cclawcore daemon               # Vollständige autonome Laufzeit starten

# Agent
cclawcore agent                # Interaktiver Chat-Modus
cclawcore agent -m "message"   # Einzelnachrichten-Modus

# Service-Verwaltung
cclawcore service install      # Als OS-Dienst installieren (launchd/systemd)
cclawcore service start|stop|restart|status

# Kanäle
cclawcore channel list         # Konfigurierte Kanäle auflisten
cclawcore channel doctor       # Kanalgesundheit prüfen
cclawcore channel bind-telegram 123456789

# Cron + Planung
cclawcore cron list            # Geplante Aufgaben auflisten
cclawcore cron add "*/5 * * * *" --prompt "Check system health"
cclawcore cron remove <id>

# Speicher
cclawcore memory list          # Speichereinträge auflisten
cclawcore memory get <key>     # Speicher abrufen
cclawcore memory stats         # Speicherstatistiken
...
# Hardware-Peripherie
cclawcore hardware discover    # Angeschlossene Geräte scannen
cclawcore peripheral list      # Angeschlossene Peripherie auflisten
cclawcore peripheral flash     # Firmware auf Gerät flashen
...
Oder überspringe die obigen Schritte und installiere alles (Systemabhängigkeiten, Rust, CclawCore) mit einem einzigen Befehl:
...
- Folge [CclawCore Onboard](#schnellstart) für eine geführte Einrichtung.
...
CclawCore wurde für den glatten Krebs 🦀 gebaut, einen schnellen und effizienten KI-Assistenten. Entwickelt von Argenis De La Rosa und der Community.

- [cclawcorelabs.ai](https://cclawcorelabs.ai)
- [@cclawcorelabs](https://x.com/cclawcorelabs)

## CclawCore unterstützen

Wenn CclawCore bei deiner Arbeit hilft und du die laufende Entwicklung unterstützen möchtest, kannst du hier spenden:
...
Neu bei CclawCore? Suche nach Issues mit dem Label [`good first issue`](https://github.com/cclawcore-labs/cclawcore/issues?q=is%3Aissue+is%3Aopen+label%3A%22good+first+issue%22) — siehe unseren [Beitragsleitfaden](CONTRIBUTING.md#first-time-contributors) für den Einstieg. KI-/Vibe-coded PRs willkommen! 🤖
...
- Neuer `Skill` → `~/.cclawcore/workspace/skills/<name>/`
...
**Dies ist das einzige offizielle CclawCore-Repository:**

> https://github.com/cclawcore-labs/cclawcore

Jedes andere Repository, jede Organisation, Domain oder jedes Paket, das behauptet, "CclawCore" zu sein oder eine Zugehörigkeit zu CclawCore Labs impliziert, ist **nicht autorisiert und nicht mit diesem Projekt verbunden**. Bekannte nicht autorisierte Forks werden in [TRADEMARK.md](docs/maintainers/trademark.md) aufgelistet.

Wenn du auf Identitätsdiebstahl oder Markenrechtsmissbrauch stößt, [eröffne bitte ein Issue](https://github.com/cclawcore-labs/cclawcore/issues).

---

## Lizenz

CclawCore ist doppelt lizenziert für maximale Offenheit und Schutz der Mitwirkenden:
...
Der **CclawCore**-Name und das Logo sind Marken von CclawCore Labs. Diese Lizenz gewährt keine Erlaubnis, sie zu verwenden, um Unterstützung oder Zugehörigkeit zu implizieren. Siehe [TRADEMARK.md](docs/maintainers/trademark.md) für erlaubte und verbotene Verwendungen.
...
**CclawCore** — Null Overhead. Null Kompromisse. Überall bereitstellen. Alles austauschen. 🦀
