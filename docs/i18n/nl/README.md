...
<h1 align="center">🦀 CclawCore — Persoonlijke AI-Assistent</h1>
...
`cclawcore onboard` wordt automatisch uitgevoerd na installatie om je workspace en provider te configureren.
...
cclawcore gateway                # standaard: 127.0.0.1:42617
cclawcore gateway --port 0       # willekeurige poort (beveiligingsversterkt)
...
Bijwerken? Voer `cclawcore doctor` uit na het updaten.
...
CclawCore kan je OpenClaw-workspace, geheugen en configuratie importeren:
...
Dit migreert je geheugenregistraties, workspace-bestanden en configuratie van `~/.openclaw/` naar `~/.cclawcore/`. Configuratie wordt automatisch geconverteerd van JSON naar TOML.

## Standaard beveiligingsinstellingen (DM-toegang)

CclawCore verbindt met echte berichtenplatforms. Behandel inkomende DM's als onbetrouwbare invoer.
...
- Goedkeuren met: `cclawcore pairing approve <channel> <code>` (vervolgens wordt de afzender toegevoegd aan een lokale allowlist).
- Publieke inkomende DM's vereisen een expliciete opt-in in `config.toml`.
- Voer `cclawcore doctor` uit om riskante of verkeerd geconfigureerde DM-beleidsregels te detecteren.
...
| ---------- | ----------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-02-19 | _Kritiek_  | We zijn **niet gelieerd** aan `openagen/cclawcore`, `cclawcore.org` of `cclawcore.net`. De domeinen `cclawcore.org` en `cclawcore.net` verwijzen momenteel naar de `openagen/cclawcore`-fork, en dat domein/repository doet zich voor als onze officiële website/project.                                                                                       | Vertrouw geen informatie, binaire bestanden, fondswerving of aankondigingen van die bronnen. Gebruik alleen [dit repository](https://github.com/cclawcore-labs/cclawcore) en onze geverifieerde sociale accounts.                                                                                                                                                                                                                                                                                                                                                                       |
...
### Waarom teams kiezen voor CclawCore
...
## Benchmark-overzicht (CclawCore vs OpenClaw, reproduceerbaar)
...
> Opmerkingen: CclawCore-resultaten zijn gemeten op release-builds met `/usr/bin/time -l`. OpenClaw vereist Node.js-runtime (typisch ~390MB extra geheugenoverhead), terwijl NanoBot Python-runtime vereist. PicoClaw en CclawCore zijn statische binaries. De RAM-cijfers hierboven zijn runtime-geheugen; compilatievereisten zijn hoger.
...
Minimale `~/.cclawcore/config.toml`:
...
CclawCore ondersteunt native abonnementsautorisatieprofielen (meerdere accounts, versleuteld in rust).

- Opslagbestand: `~/.cclawcore/auth-profiles.json`
- Versleutelingssleutel: `~/.cclawcore/.secret_key`
...
Workspace-root: `~/.cclawcore/workspace/` (configureerbaar via config).
...
Skills: `~/.cclawcore/workspace/skills/<skill>/SKILL.md` of `SKILL.toml`.
...
```bash
# Workspace-beheer
cclawcore onboard              # Begeleide installatiewizard
cclawcore status               # Toon daemon/agent-status
cclawcore doctor               # Voer systeemdiagnostiek uit

# Gateway + daemon
cclawcore gateway              # Start gateway-server (127.0.0.1:42617)
cclawcore daemon               # Start volledige autonome runtime

# Agent
cclawcore agent                # Interactieve chatmodus
cclawcore agent -m "message"   # Enkele berichtmodus

# Servicebeheer
cclawcore service install      # Installeer als OS-service (launchd/systemd)
cclawcore service start|stop|restart|status

# Kanalen
cclawcore channel list         # Lijst geconfigureerde kanalen
cclawcore channel doctor       # Controleer kanaalgezondheid
cclawcore channel bind-telegram 123456789

# Cron + planning
cclawcore cron list            # Lijst geplande taken
cclawcore cron add "*/5 * * * *" --prompt "Check system health"
cclawcore cron remove <id>

# Geheugen
cclawcore memory list          # Lijst geheugenregistraties
cclawcore memory get <key>     # Haal een geheugenitem op
cclawcore memory stats         # Geheugenstatistieken
...
cclawcore hardware discover    # Scan verbonden apparaten
cclawcore peripheral list      # Lijst verbonden randapparatuur
cclawcore peripheral flash     # Flash firmware naar apparaat
...
Of sla bovenstaande stappen over en installeer alles (systeemafhankelijkheden, Rust, CclawCore) in één commando:
...
- Volg [CclawCore Onboard](#snelle-start) voor een begeleide setup.
...
CclawCore is gebouwd voor de smooth crab 🦀, een snelle en efficiënte AI-assistent. Gebouwd door Argenis De La Rosa en de gemeenschap.
...
Als CclawCore je werk helpt en je de voortdurende ontwikkeling wilt steunen, kun je hier doneren:
...
Nieuw bij CclawCore? Zoek naar issues gelabeld [`good first issue`](https://github.com/cclawcore-labs/cclawcore/issues?q=is%3Aissue+is%3Aopen+label%3A%22good+first+issue%22) — zie onze [Bijdragegids](CONTRIBUTING.md#first-time-contributors) om te beginnen. AI/vibe-coded PR's welkom! 🤖
...
- Nieuwe `Skill` → `~/.cclawcore/workspace/skills/<name>/`
...
**Dit is het enige officiële CclawCore-repository:**

> https://github.com/cclawcore-labs/cclawcore

Elk ander repository, organisatie, domein of pakket dat beweert "CclawCore" te zijn of een relatie met CclawCore Labs impliceert, is **ongeautoriseerd en niet gelieerd aan dit project**. Bekende ongeautoriseerde forks worden vermeld in [TRADEMARK.md](docs/maintainers/trademark.md).

Als je imitatie of merkmisbruik tegenkomt, [open dan een issue](https://github.com/cclawcore-labs/cclawcore/issues).

---

## Licentie

CclawCore heeft een dubbele licentie voor maximale openheid en bescherming van bijdragers:
...
De **CclawCore**-naam en het logo zijn handelsmerken van CclawCore Labs. Deze licentie verleent geen toestemming om ze te gebruiken om goedkeuring of affiliatie te impliceren. Zie [TRADEMARK.md](docs/maintainers/trademark.md) voor toegestaan en verboden gebruik.
...
**CclawCore** — Nul overhead. Nul compromis. Implementeer overal. Wissel alles. 🦀
