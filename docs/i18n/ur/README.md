...
<h1 align="center">🦀 CclawCore — ذاتی AI اسسٹنٹ</h1>
...
> **تجویز کردہ سیٹ اپ:** اپنے ٹرمینل میں `cclawcore onboard` چلائیں۔ CclawCore Onboard آپ کو gateway، workspace، چینلز، اور provider ترتیب دینے میں مرحلہ وار رہنمائی کرتا ہے۔ یہ تجویز کردہ سیٹ اپ راستہ ہے اور macOS، Linux، اور Windows (WSL2 کے ذریعے) پر کام کرتا ہے۔ نئی تنصیب؟ یہاں سے شروع کریں: [شروع کریں](#فوری-آغاز)
...
`cclawcore onboard` انسٹال کے بعد خود بخود چلتا ہے تاکہ آپ کا workspace اور provider ترتیب دیا جا سکے۔
...
cclawcore gateway                # ڈیفالٹ: 127.0.0.1:42617
cclawcore gateway --port 0       # بے ترتیب پورٹ (سیکیورٹی مضبوط)
...
اپ گریڈ کر رہے ہیں؟ اپ ڈیٹ کے بعد `cclawcore doctor` چلائیں۔
...
CclawCore آپ کا OpenClaw workspace، میموری، اور کنفیگریشن درآمد کر سکتا ہے:
...
یہ آپ کے میموری اندراجات، workspace فائلیں، اور کنفیگریشن `~/.openclaw/` سے `~/.cclawcore/` میں منتقل کرتا ہے۔ کنفیگ خود بخود JSON سے TOML میں تبدیل ہو جاتی ہے۔

## سیکیورٹی ڈیفالٹس (DM رسائی)

CclawCore حقیقی پیغام رسانی سطحوں سے جڑتا ہے۔ آنے والے DMs کو غیر بھروسہ مند ان پٹ سمجھیں۔
...
- منظوری دیں: `cclawcore pairing approve <channel> <code>` (پھر بھیجنے والا مقامی اجازت نامہ میں شامل ہو جاتا ہے)۔
- عوامی آنے والے DMs کے لیے `config.toml` میں واضح opt-in ضروری ہے۔
- خطرناک یا غلط ترتیب شدہ DM پالیسیوں کا پتہ لگانے کے لیے `cclawcore doctor` چلائیں۔
...
| ---------- | ----------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-02-19 | _اہم ترین_  | ہم `openagen/cclawcore`، `cclawcore.org` یا `cclawcore.net` سے **وابستہ نہیں** ہیں۔ `cclawcore.org` اور `cclawcore.net` ڈومینز فی الحال `openagen/cclawcore` فورک کی طرف اشارہ کرتے ہیں، اور وہ ڈومین/ریپوزٹری ہماری سرکاری ویب سائٹ/پروجیکٹ کی نقل کر رہے ہیں۔                                                                                       | ان ذرائع سے معلومات، بائنریز، فنڈ ریزنگ، یا اعلانات پر بھروسہ نہ کریں۔ صرف [یہ ریپوزٹری](https://github.com/cclawcore-labs/cclawcore) اور ہمارے تصدیق شدہ سوشل اکاؤنٹس استعمال کریں۔                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
## بینچ مارک سنیپ شاٹ (CclawCore بمقابلہ OpenClaw، قابل تکرار)
...
کم از کم `~/.cclawcore/config.toml`:
...
CclawCore سبسکرپشن نیٹو auth پروفائلز سپورٹ کرتا ہے (ملٹی اکاؤنٹ، آرام پر خفیہ)۔

- اسٹور فائل: `~/.cclawcore/auth-profiles.json`
- خفیہ کاری کلید: `~/.cclawcore/.secret_key`
...
Workspace روٹ: `~/.cclawcore/workspace/` (config کے ذریعے قابل ترتیب)۔
...
اسکلز: `~/.cclawcore/workspace/skills/<skill>/SKILL.md` یا `SKILL.toml`۔
...
```bash
# Workspace مینجمنٹ
cclawcore onboard              # رہنمائی شدہ سیٹ اپ وزرڈ
cclawcore status               # daemon/ایجنٹ اسٹیٹس دکھائیں
cclawcore doctor               # سسٹم تشخیص چلائیں

# Gateway + daemon
cclawcore gateway              # Gateway سرور شروع کریں (127.0.0.1:42617)
cclawcore daemon               # مکمل خودمختار رن ٹائم شروع کریں

# ایجنٹ
cclawcore agent                # انٹرایکٹو چیٹ موڈ
cclawcore agent -m "message"   # واحد پیغام موڈ

# سروس مینجمنٹ
cclawcore service install      # OS سروس کے طور پر انسٹال کریں (launchd/systemd)
cclawcore service start|stop|restart|status

# چینلز
cclawcore channel list         # ترتیب شدہ چینلز کی فہرست
cclawcore channel doctor       # چینل صحت چیک کریں
cclawcore channel bind-telegram 123456789

# Cron + شیڈولنگ
cclawcore cron list            # شیڈولڈ جابز کی فہرست
cclawcore cron add "*/5 * * * *" --prompt "Check system health"
cclawcore cron remove <id>

# میموری
cclawcore memory list          # میموری اندراجات کی فہرست
cclawcore memory get <key>     # میموری حاصل کریں
cclawcore memory stats         # میموری اعداد و شمار
...
# ہارڈویئر پیری فیرلز
cclawcore hardware discover    # منسلک آلات اسکین کریں
cclawcore peripheral list      # منسلک پیری فیرلز کی فہرست
cclawcore peripheral flash     # آلے پر فرم ویئر فلیش کریں
...
یا اوپر کے مراحل چھوڑیں اور سب کچھ (سسٹم dependencies، Rust، CclawCore) ایک کمانڈ میں انسٹال کریں:
...
- رہنمائی شدہ سیٹ اپ کے لیے [CclawCore Onboard](#فوری-آغاز) فالو کریں۔
...
CclawCore smooth crab 🦀 کے لیے بنایا گیا تھا، ایک تیز اور مؤثر AI اسسٹنٹ۔ Argenis De La Rosa اور کمیونٹی نے بنایا۔

- [cclawcorelabs.ai](https://cclawcorelabs.ai)
- [@cclawcorelabs](https://x.com/cclawcorelabs)

## CclawCore کی حمایت کریں

اگر CclawCore آپ کے کام میں مدد کرتا ہے اور آپ جاری ترقی کی حمایت کرنا چاہتے ہیں، تو آپ یہاں عطیہ دے سکتے ہیں:
...
CclawCore میں نئے ہیں؟ [`good first issue`](https://github.com/cclawcore-labs/cclawcore/issues?q=is%3Aissue+is%3Aopen+label%3A%22good+first+issue%22) لیبل والے issues تلاش کریں — شروع کرنے کے طریقے کے لیے [شراکت گائیڈ](CONTRIBUTING.md#first-time-contributors) دیکھیں۔ AI/vibe-coded PRs کا خیرمقدم ہے! 🤖
...
- نیا `Skill` → `~/.cclawcore/workspace/skills/<name>/`
...
**یہ CclawCore کی واحد سرکاری ریپوزٹری ہے:**

> https://github.com/cclawcore-labs/cclawcore

کوئی بھی دوسری ریپوزٹری، تنظیم، ڈومین، یا پیکیج جو "CclawCore" ہونے کا دعویٰ کرے یا CclawCore Labs سے وابستگی کا اشارہ کرے **غیر مجاز ہے اور اس پروجیکٹ سے وابستہ نہیں ہے**۔ معلوم غیر مجاز فورکس [TRADEMARK.md](docs/maintainers/trademark.md) میں درج ہوں گے۔

اگر آپ کو نقل یا ٹریڈ مارک کا غلط استعمال ملے، براہ کرم [issue کھولیں](https://github.com/cclawcore-labs/cclawcore/issues)۔

---

## لائسنس

CclawCore زیادہ سے زیادہ کشادگی اور شراکت دار تحفظ کے لیے دوہری لائسنس یافتہ ہے:
...
**CclawCore** نام اور لوگو CclawCore Labs کے ٹریڈ مارکس ہیں۔ یہ لائسنس انہیں توثیق یا وابستگی کا اشارہ دینے کے لیے استعمال کرنے کی اجازت نہیں دیتا۔ مجاز اور ممنوع استعمال کے لیے [TRADEMARK.md](docs/maintainers/trademark.md) دیکھیں۔
...
**CclawCore** — صفر اوور ہیڈ۔ صفر سمجھوتا۔ کہیں بھی تعینات کریں۔ کچھ بھی تبدیل کریں۔ 🦀
