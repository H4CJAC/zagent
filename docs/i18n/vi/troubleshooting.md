...
- `cargo check` / `cargo build` dừng lâu ở `Checking cclawcore`
...
### Không tìm thấy lệnh `cclawcore` sau cài đặt

Triệu chứng:

- Cài đặt thành công nhưng shell không tìm thấy `cclawcore`
...
Xác minh `~/.cclawcore/config.toml`:
...
- Dừng các tiến trình `cclawcore daemon` / `cclawcore channel start` thừa
