# CclawCore trên Arduino Uno Q — Hướng dẫn từng bước

Chạy CclawCore trên phía Linux của Arduino Uno Q. Telegram hoạt động qua WiFi; điều khiển GPIO dùng Bridge (yêu cầu một ứng dụng App Lab tối giản).

---

## Những gì đã có sẵn (Không cần thay đổi code)

CclawCore bao gồm mọi thứ cần thiết cho Arduino Uno Q. **Clone repo và làm theo hướng dẫn này — không cần patch hay code tùy chỉnh nào.**
...
| Setup command | `src/peripherals/uno_q_setup.rs` | `cclawcore peripheral setup-uno-q` triển khai Bridge qua scp + arduino-app-cli |
...
## Phase 2: Cài đặt CclawCore trên Uno Q
...
# Clone cclawcore (hoặc scp project của bạn)
...
# Cài đặt
sudo cp target/release/cclawcore /usr/local/bin/
```
...
# Copy sang Uno Q
scp target/aarch64-unknown-linux-gnu/release/cclawcore arduino@<UNO_Q_IP>:~/
ssh arduino@<UNO_Q_IP> "sudo mv ~/cclawcore /usr/local/bin/"
```
...
# Cấu hình nhanh
cclawcore onboard --api-key YOUR_OPENROUTER_KEY --provider openrouter

# Hoặc tạo config thủ công
mkdir -p ~/.cclawcore/workspace
nano ~/.cclawcore/config.toml
```
...
**Tại bước này:** Telegram chat hoạt động. Gửi tin nhắn tới bot — CclawCore phản hồi. Chưa có GPIO.

---

## Phase 5: GPIO qua Bridge (CclawCore xử lý tự động)

CclawCore bao gồm Bridge app và setup command.

### 5.1 Triển khai Bridge App

**Từ Mac** (với repo cclawcore):
```bash
cclawcore peripheral setup-uno-q --host 192.168.0.48
```

**Từ Uno Q** (đã SSH vào):
```bash
cclawcore peripheral setup-uno-q
```
...
Giờ khi bạn nhắn tin cho Telegram bot *"Turn on the LED"* hoặc *"Set pin 13 high"*, CclawCore dùng `gpio_write` qua Bridge.
...
| 5 | `git clone https://github.com/cclawcore-labs/cclawcore.git && cd cclawcore` |
| 6 | `cargo build --release --no-default-features` |
| 7 | `cclawcore onboard --api-key KEY --provider openrouter` |
| 8 | Chỉnh sửa `~/.cclawcore/config.toml` (thêm Telegram bot_token) |
| 9 | `cclawcore daemon --host 127.0.0.1 --port 3000` |
...
- **"command not found: cclawcore"** — Dùng đường dẫn đầy đủ: `/usr/local/bin/cclawcore` hoặc đảm bảo `~/.cargo/bin` nằm trong PATH.
...
- **Lệnh GPIO bị bỏ qua** — Đảm bảo Bridge app đang chạy (`cclawcore peripheral setup-uno-q` triển khai và khởi động nó). Config phải có `board = "arduino-uno-q"` và `transport = "bridge"`.
- **LLM provider (GLM/Zhipu)** — Dùng `default_provider = "glm"` hoặc `"zhipu"` với `GLM_API_KEY` trong env hoặc config. CclawCore dùng endpoint v4 chính xác.
