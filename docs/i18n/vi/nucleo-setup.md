# CclawCore trên Nucleo-F401RE — Hướng dẫn từng bước

Chạy CclawCore trên Mac hoặc Linux. Kết nối Nucleo-F401RE qua USB. Điều khiển GPIO (LED, các pin) qua Telegram hoặc CLI.

---

## Lấy thông tin board qua Telegram (Không cần nạp firmware)

CclawCore có thể đọc thông tin chip từ Nucleo qua USB **mà không cần nạp firmware nào**. Nhắn tin cho Telegram bot của bạn:
...
CclawCore bao gồm mọi thứ cần thiết cho Nucleo-F401RE:
...
| Flash command | `cclawcore peripheral flash-nucleo` | Build firmware, nạp qua probe-rs |
...
### 1.2 Nạp qua CclawCore

Từ thư mục gốc của repo cclawcore:
...
Thêm vào `~/.cclawcore/config.toml`:
...
| 5 | `cclawcore daemon` hoặc `cclawcore agent -m "Turn on LED"` |
...
- **Lệnh GPIO bị bỏ qua** — Kiểm tra `path` trong config có khớp với serial port của bạn. Chạy `cclawcore peripheral list` để xác nhận.
