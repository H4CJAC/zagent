...
[INFO] ✓ Audit logging enabled (~/.config/cclawcore/audit.log)
...
$ cclawcore security --status
Security Status:
  ✓ Sandbox: Landlock (Linux kernel 6.2)
  ✓ Memory monitoring: 512MB limit
  ✓ Audit logging: ~/.config/cclawcore/audit.log
  → 47 events logged today

# Tắt sandbox tường minh (ghi vào config)
$ cclawcore config set security.sandbox.enabled false

# Bật backend cụ thể
$ cclawcore config set security.sandbox.backend firejail

# Điều chỉnh giới hạn
$ cclawcore config set security.resources.max_memory_mb 2048
```
...
✅ **Hiển thị trạng thái** — `cclawcore security --status`
