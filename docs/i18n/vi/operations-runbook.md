...
|---|---|---|
| Foreground runtime | `cclawcore daemon` | gỡ lỗi cục bộ, phiên ngắn |
| Foreground gateway only | `cclawcore gateway` | kiểm thử webhook endpoint |
| User service | `cclawcore service install && cclawcore service start` | runtime được quản lý liên tục bởi operator |
...
|---|---|---|
| Tính hợp lệ của config | `cclawcore doctor` | không có lỗi nghiêm trọng |
| Kết nối channel | `cclawcore channel doctor` | các channel đã cấu hình đều khoẻ mạnh |
| Tóm tắt runtime | `cclawcore status` | provider/model/channels như mong đợi |
| Heartbeat/trạng thái daemon | `~/.cclawcore/daemon_state.json` | file được cập nhật định kỳ |
...
4. Nếu các channel vẫn thất bại, kiểm tra allowlist và thông tin xác thực trong `~/.cclawcore/config.toml`.
...
1. sao lưu `~/.cclawcore/config.toml`
2. chỉ áp dụng một thay đổi logic tại một thời điểm
3. chạy `cclawcore doctor`
