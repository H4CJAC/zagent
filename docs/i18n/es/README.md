...
<h1 align="center">🦀 CclawCore — Asistente Personal de IA</h1>
...
> **Configuración recomendada:** ejecuta `cclawcore onboard` en tu terminal. CclawCore Onboard te guía paso a paso en la configuración del gateway, workspace, canales y proveedor. Es la ruta de configuración recomendada y funciona en macOS, Linux y Windows (vía WSL2). ¿Nueva instalación? Empieza aquí: [Primeros pasos](#inicio-rápido)
...
`cclawcore onboard` se ejecuta automáticamente después de la instalación para configurar tu workspace y proveedor.
...
cclawcore gateway                # por defecto: 127.0.0.1:42617
cclawcore gateway --port 0       # puerto aleatorio (seguridad reforzada)
...
¿Actualizando? Ejecuta `cclawcore doctor` después de actualizar.
...
CclawCore puede importar tu workspace, memoria y configuración de OpenClaw:
...
Esto migra tus entradas de memoria, archivos del workspace y configuración de `~/.openclaw/` a `~/.cclawcore/`. La configuración se convierte de JSON a TOML automáticamente.

## Valores predeterminados de seguridad (acceso por DM)

CclawCore se conecta a superficies de mensajería reales. Trata los DMs entrantes como entrada no confiable.
...
- Aprobar con: `cclawcore pairing approve <channel> <code>` (luego el remitente se agrega a una lista de permitidos local).
- Los DMs públicos entrantes requieren una activación explícita en `config.toml`.
- Ejecuta `cclawcore doctor` para detectar políticas de DM riesgosas o mal configuradas.
...
| ---------- | ----------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-02-19 | _Crítico_  | **No estamos afiliados** con `openagen/cclawcore`, `cclawcore.org` ni `cclawcore.net`. Los dominios `cclawcore.org` y `cclawcore.net` actualmente apuntan al fork `openagen/cclawcore`, y ese dominio/repositorio están suplantando nuestro sitio web/proyecto oficial.                                                                                       | No confíes en información, binarios, recaudaciones de fondos o anuncios de esas fuentes. Usa solo [este repositorio](https://github.com/cclawcore-labs/cclawcore) y nuestras cuentas sociales verificadas.                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
### Por qué los equipos eligen CclawCore
...
## Resumen de benchmarks (CclawCore vs OpenClaw, reproducible)
...
> Notas: Los resultados de CclawCore se miden en compilaciones release usando `/usr/bin/time -l`. OpenClaw requiere el runtime de Node.js (típicamente ~390MB de sobrecarga adicional de memoria), mientras que NanoBot requiere el runtime de Python. PicoClaw y CclawCore son binarios estáticos. Las cifras de RAM anteriores son de memoria en runtime; los requisitos de compilación son mayores.
...
CclawCore soporta perfiles de autenticación nativos de suscripción (multi-cuenta, cifrados en reposo).

- Archivo de almacenamiento: `~/.cclawcore/auth-profiles.json`
- Clave de cifrado: `~/.cclawcore/.secret_key`
...
Raíz del workspace: `~/.cclawcore/workspace/` (configurable vía config).
...
Skills: `~/.cclawcore/workspace/skills/<skill>/SKILL.md` o `SKILL.toml`.
...
cclawcore onboard              # Asistente de configuración guiada
cclawcore status               # Mostrar estado del daemon/agente
cclawcore doctor               # Ejecutar diagnósticos del sistema

# Gateway + daemon
cclawcore gateway              # Iniciar servidor gateway (127.0.0.1:42617)
cclawcore daemon               # Iniciar runtime autónomo completo

# Agente
cclawcore agent                # Modo de chat interactivo
cclawcore agent -m "message"   # Modo de mensaje único

# Gestión de servicios
cclawcore service install      # Instalar como servicio del SO (launchd/systemd)
cclawcore service start|stop|restart|status

# Canales
cclawcore channel list         # Listar canales configurados
cclawcore channel doctor       # Verificar salud de los canales
cclawcore channel bind-telegram 123456789

# Cron + programación
cclawcore cron list            # Listar trabajos programados
cclawcore cron add "*/5 * * * *" --prompt "Check system health"
cclawcore cron remove <id>

# Memoria
cclawcore memory list          # Listar entradas de memoria
cclawcore memory get <key>     # Recuperar una memoria
cclawcore memory stats         # Estadísticas de memoria
...
cclawcore hardware discover    # Escanear dispositivos conectados
cclawcore peripheral list      # Listar periféricos conectados
cclawcore peripheral flash     # Flashear firmware al dispositivo
...
O salta los pasos anteriores e instala todo (dependencias del sistema, Rust, CclawCore) en un solo comando:
...
- Sigue [CclawCore Onboard](#inicio-rápido) para una configuración guiada.
...
CclawCore fue construido para el cangrejo suave 🦀, un asistente de IA rápido y eficiente. Construido por Argenis De La Rosa y la comunidad.
...
Si CclawCore ayuda en tu trabajo y quieres apoyar el desarrollo continuo, puedes donar aquí:
...
¿Nuevo en CclawCore? Busca issues etiquetados como [`good first issue`](https://github.com/cclawcore-labs/cclawcore/issues?q=is%3Aissue+is%3Aopen+label%3A%22good+first+issue%22) — consulta nuestra [Guía de contribución](CONTRIBUTING.md#first-time-contributors) para saber cómo empezar. ¡PRs con IA/vibe-coded son bienvenidos! 🤖
...
- Nuevo `Skill` → `~/.cclawcore/workspace/skills/<name>/`
...
**Este es el único repositorio oficial de CclawCore:**

> https://github.com/cclawcore-labs/cclawcore

Cualquier otro repositorio, organización, dominio o paquete que afirme ser "CclawCore" o implique afiliación con CclawCore Labs **no está autorizado y no está afiliado con este proyecto**. Los forks no autorizados conocidos se listarán en [TRADEMARK.md](docs/maintainers/trademark.md).

Si encuentras suplantación o uso indebido de marca, por favor [abre un issue](https://github.com/cclawcore-labs/cclawcore/issues).

---

## Licencia

CclawCore tiene doble licencia para máxima apertura y protección de los contribuidores:
...
El nombre y logo de **CclawCore** son marcas registradas de CclawCore Labs. Esta licencia no otorga permiso para usarlos para implicar respaldo o afiliación. Ver [TRADEMARK.md](docs/maintainers/trademark.md) para usos permitidos y prohibidos.
...
**CclawCore** — Cero sobrecarga. Cero compromisos. Despliega en cualquier lugar. Intercambia cualquier cosa. 🦀
