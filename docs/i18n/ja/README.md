...
<h1 align="center">🦀 CclawCore — パーソナルAIアシスタント</h1>
...
> **推奨セットアップ：** ターミナルで `cclawcore onboard` を実行してください。CclawCore Onboardがゲートウェイ、ワークスペース、チャンネル、プロバイダーのセットアップをステップバイステップでガイドします。これは推奨されるセットアップパスで、macOS、Linux、Windows（WSL2経由）で動作します。新規インストール？ここから開始：[はじめに](#クイックスタートtldr)
...
`cclawcore onboard` はインストール後に自動的に実行され、ワークスペースとプロバイダーを設定します。
...
cclawcore gateway                # デフォルト：127.0.0.1:42617
cclawcore gateway --port 0       # ランダムポート（セキュリティ強化）
...
アップグレード？更新後に `cclawcore doctor` を実行してください。
...
CclawCoreはOpenClawのワークスペース、メモリ、設定をインポートできます：
...
これにより、メモリエントリ、ワークスペースファイル、設定が `~/.openclaw/` から `~/.cclawcore/` に移行されます。設定はJSONからTOMLに自動変換されます。

## セキュリティデフォルト（DMアクセス）

CclawCoreは実際のメッセージングサービスに接続します。着信DMを信頼できない入力として扱ってください。
...
- 承認方法：`cclawcore pairing approve <channel> <code>`（送信者がローカル許可リストに追加されます）。
- パブリック着信DMには `config.toml` での明示的なオプトインが必要です。
- `cclawcore doctor` を実行してリスクのある、または設定ミスのあるDMポリシーを検出します。
...
| 日付 (UTC) | レベル | 通知 | 対応 |
| ---------- | ------ | ---- | ---- |
| 2026-02-19 | _重大_ | 当プロジェクトは `openagen/cclawcore`、`cclawcore.org`、`cclawcore.net` とは**一切関係ありません**。`cclawcore.org` と `cclawcore.net` ドメインは現在 `openagen/cclawcore` フォークを指しており、そのドメイン/リポジトリは当プロジェクトの公式ウェブサイト/プロジェクトを偽装しています。 | それらのソースからの情報、バイナリ、資金調達、告知を信頼しないでください。[このリポジトリ](https://github.com/cclawcore-labs/cclawcore)と認証済みのソーシャルアカウントのみを使用してください。 |
...
## ベンチマークスナップショット（CclawCore vs OpenClaw、再現可能）
...
> 注意：CclawCoreの結果はリリースビルドで `/usr/bin/time -l` を使用して測定されています。OpenClawにはNode.jsランタイム（通常約390MBの追加メモリオーバーヘッド）が必要で、NanoBotにはPythonランタイムが必要です。PicoClawとCclawCoreは静的バイナリです。上記のRAM数値はランタイムメモリです。ビルド時のコンパイル要件はより高くなります。
...
CclawCoreはサブスクリプションネイティブ認証プロファイル（マルチアカウント、保存時暗号化）をサポートしています。

- ストアファイル：`~/.cclawcore/auth-profiles.json`
- 暗号化キー：`~/.cclawcore/.secret_key`
...
ワークスペースルート：`~/.cclawcore/workspace/`（設定で変更可能）。
...
スキル：`~/.cclawcore/workspace/skills/<skill>/SKILL.md` または `SKILL.toml`。
...
```bash
# ワークスペース管理
cclawcore onboard              # ガイド付きセットアップウィザード
cclawcore status               # デーモン/エージェントのステータス表示
cclawcore doctor               # システム診断を実行

# ゲートウェイ + デーモン
cclawcore gateway              # ゲートウェイサーバーを起動（127.0.0.1:42617）
cclawcore daemon               # フル自律ランタイムを起動

# エージェント
cclawcore agent                # インタラクティブチャットモード
cclawcore agent -m "message"   # 単一メッセージモード

# サービス管理
cclawcore service install      # OSサービスとしてインストール（launchd/systemd）
cclawcore service start|stop|restart|status

# チャンネル
cclawcore channel list         # 設定済みチャンネルの一覧
cclawcore channel doctor       # チャンネルヘルスの確認
cclawcore channel bind-telegram 123456789

# Cron + スケジューリング
cclawcore cron list            # スケジュールタスクの一覧
cclawcore cron add "*/5 * * * *" --prompt "Check system health"
cclawcore cron remove <id>

# メモリ
cclawcore memory list          # メモリエントリの一覧
cclawcore memory get <key>     # メモリの取得
cclawcore memory stats         # メモリ統計
...
# ハードウェア周辺機器
cclawcore hardware discover    # 接続デバイスのスキャン
cclawcore peripheral list      # 接続周辺機器の一覧
cclawcore peripheral flash     # デバイスへのファームウェア書き込み
...
または、上記のステップをスキップして、単一コマンドですべてをインストール（システム依存、Rust、CclawCore）：
...
- [CclawCore Onboard](#クイックスタートtldr)でガイド付きセットアップ。
...
CclawCoreはsmooth crab 🦀のために構築されました。高速で効率的なAIアシスタント。Argenis De La Rosaとコミュニティによって構築されました。
...
CclawCoreがあなたの仕事に役立ち、継続的な開発を支援したい場合は、こちらから寄付できます：
...
CclawCore初心者ですか？[`good first issue`](https://github.com/cclawcore-labs/cclawcore/issues?q=is%3Aissue+is%3Aopen+label%3A%22good+first+issue%22) ラベルの付いた課題を探してください — 始め方は[貢献ガイド](CONTRIBUTING.md#first-time-contributors)を参照。AI/vibe-coded PRも歓迎します！🤖
...
- 新 `Skill` → `~/.cclawcore/workspace/skills/<name>/`
...
**これがCclawCoreの唯一の公式リポジトリです：**

> https://github.com/cclawcore-labs/cclawcore

「CclawCore」を名乗る、またはCclawCore Labsとの提携を示唆する他のリポジトリ、組織、ドメイン、パッケージは**無許可であり、本プロジェクトとは無関係です**。既知の無許可フォークは [TRADEMARK.md](docs/maintainers/trademark.md) に記載されます。

偽装や商標の悪用を見つけた場合は、[issueを作成](https://github.com/cclawcore-labs/cclawcore/issues)してください。

---

## ライセンス

CclawCoreは最大限のオープン性と貢献者保護のためにデュアルライセンスです：
...
**CclawCore** の名称とロゴはCclawCore Labsの商標です。このライセンスは、推薦や提携を暗示するための使用許可を付与しません。許可された使用と禁止された使用については [TRADEMARK.md](docs/maintainers/trademark.md) を参照してください。
...
**CclawCore** — ゼロオーバーヘッド。ゼロ妥協。どこでもデプロイ。何でも交換。🦀
