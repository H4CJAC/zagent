...
> **Anbefalet opsaetning:** kor `cclawcore onboard` i din terminal. CclawCore Onboard guider dig trin for trin gennem opsaetning af gateway, arbejdsomrade, kanaler og udbyder. Det er den anbefalede opsaetningssti og virker pa macOS, Linux og Windows (via WSL2). Ny installation? Start her: [Kom i gang](#hurtig-start-tldr)
...
`cclawcore onboard` koerer automatisk efter installation for at konfigurere dit arbejdsomrade og din udbyder.
...
cclawcore gateway --port 0       # tilfaeldig port (sikkerhedshaerdet)
...
Opgradering? Koer `cclawcore doctor` efter opdatering.
...
CclawCore kan importere dit OpenClaw-arbejdsomrade, hukommelse og konfiguration:
...
Dette migrerer dine hukommelsesposter, arbejdsomradefiler og konfiguration fra `~/.openclaw/` til `~/.cclawcore/`. Konfiguration konverteres automatisk fra JSON til TOML.

## Sikkerhedsstandarder (DM-adgang)

CclawCore forbinder til rigtige beskedplatforme. Behandl indgaaende DM'er som utrovaerdigt input.
...
- Godkend med: `cclawcore pairing approve <channel> <code>` (derefter tilfojes afsenderen til en lokal godkendelsesliste).
- Offentlige indgaaende DM'er kraever et eksplicit opt-in i `config.toml`.
- Koer `cclawcore doctor` for at afsloere risikable eller forkert konfigurerede DM-politikker.
...
| ---------- | ----------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-02-19 | _Kritisk_ | Vi er **ikke tilknyttet** `openagen/cclawcore`, `cclawcore.org` eller `cclawcore.net`. Domaenerne `cclawcore.org` og `cclawcore.net` peger i oejeblikket pa `openagen/cclawcore`-forken, og det domaene/repository udgiver sig for at vaere vores officielle hjemmeside/projekt. | Stol ikke pa information, binaerfiler, fundraising eller meddelelser fra disse kilder. Brug kun [dette repository](https://github.com/cclawcore-labs/cclawcore) og vores verificerede sociale konti. |
...
### Hvorfor hold vaelger CclawCore
...
## Benchmark-overblik (CclawCore vs OpenClaw, Reproducerbart)
...
> Notat: CclawCore-resultater er maalt pa release-builds ved brug af `/usr/bin/time -l`. OpenClaw kraever Node.js-koerselsmiljoe (typisk ~390MB ekstra hukommelsesoverhead), mens NanoBot kraever Python-koerselsmiljoe. PicoClaw og CclawCore er statiske binaerer. RAM-tallene ovenfor er koerselstidshukommelse; kompileringstidskrav er hoejere.
...
CclawCore understoetter abonnements-native godkendelsesprofiler (flere konti, krypteret i hvile).

- Lagerfil: `~/.cclawcore/auth-profiles.json`
- Krypteringsnoegle: `~/.cclawcore/.secret_key`
...
Arbejdsomraderod: `~/.cclawcore/workspace/` (konfigurerbart via config).
...
Faerdigheder: `~/.cclawcore/workspace/skills/<skill>/SKILL.md` eller `SKILL.toml`.
...
cclawcore onboard              # Guidet opsaetningsguide
cclawcore status               # Vis daemon/agent-status
cclawcore doctor               # Koer systemdiagnostik

# Gateway + daemon
cclawcore gateway              # Start gateway-server (127.0.0.1:42617)
cclawcore daemon               # Start fuld autonom koersel

# Agent
cclawcore agent                # Interaktiv chattilstand
cclawcore agent -m "message"   # Enkeltbeskedtilstand

# Servicestyring
cclawcore service install      # Installer som OS-service (launchd/systemd)
cclawcore service start|stop|restart|status

# Kanaler
cclawcore channel list         # List konfigurerede kanaler
cclawcore channel doctor       # Tjek kanalsundhed
cclawcore channel bind-telegram 123456789

# Cron + planlaegning
cclawcore cron list            # List planlagte opgaver
cclawcore cron add "*/5 * * * *" --prompt "Check system health"
cclawcore cron remove <id>

# Hukommelse
cclawcore memory list          # List hukommelsesposter
cclawcore memory get <key>     # Hent en hukommelse
cclawcore memory stats         # Hukommelsesstatistik
...
# Hardware-periferier
cclawcore hardware discover    # Skan efter tilsluttede enheder
cclawcore peripheral list      # List tilsluttede periferier
cclawcore peripheral flash     # Flash firmware til enhed
...
Eller spring trinnene ovenfor over og installer alt (systemafhaengigheder, Rust, CclawCore) med en enkelt kommando:
...
- Foelg [CclawCore Onboard](#hurtig-start-tldr) for en guidet opsaetning.
...
CclawCore blev bygget til smooth crab 🦀, en hurtig og effektiv AI-assistent. Bygget af Argenis De La Rosa og faellesskabet.
...
Hvis CclawCore hjaelper dit arbejde, og du vil stoette den igangvaerende udvikling, kan du donere her:
...
Ny til CclawCore? Kig efter issues maerket [`good first issue`](https://github.com/cclawcore-labs/cclawcore/issues?q=is%3Aissue+is%3Aopen+label%3A%22good+first+issue%22) — se vores [Bidragsguide](CONTRIBUTING.md#first-time-contributors) for at komme i gang. AI/vibe-kodede PR'er velkomne! 🤖
...
**Dette er det eneste officielle CclawCore-repository:**

> https://github.com/cclawcore-labs/cclawcore

Ethvert andet repository, organisation, domaene eller pakke, der haevder at vaere "CclawCore" eller antyder tilknytning til CclawCore Labs, er **uautoriseret og ikke tilknyttet dette projekt**. Kendte uautoriserede forks vil blive opfoert i [TRADEMARK.md](docs/maintainers/trademark.md).

Hvis du stoeder pa identitetstyveri eller varemaerkemisbrug, bedes du [aabne et issue](https://github.com/cclawcore-labs/cclawcore/issues).

---

## Licens

CclawCore er dobbeltlicenseret for maksimal aabenhed og bidragyderbeskyttelse:
...
Navnet **CclawCore** og logoet er varemaerker tilhoerende CclawCore Labs. Denne licens giver ikke tilladelse til at bruge dem til at antyde stoette eller tilknytning. Se [TRADEMARK.md](docs/maintainers/trademark.md) for tilladte og forbudte anvendelser.
...
**CclawCore** — Nul overhead. Nul kompromis. Udrulning overalt. Udskift hvad som helst. 🦀
