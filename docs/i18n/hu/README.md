...
<h1 align="center">🦀 CclawCore — Szemelyes MI Asszisztens</h1>
...
<p align="center">
  <a href="https://cclawcorelabs.ai">Weboldal</a> ·
...
> **Ajanlott beallitas:** futtasd a `cclawcore onboard` parancsot a terminalban. A CclawCore Onboard lepesrol lepesre vegigvezet a gateway, munkater, csatornak es szolgaltato beallitasan. Ez az ajanlott beallitasi ut, es mukodik macOS-en, Linuxon es Windowson (WSL2-n keresztul). Uj telepites? Kezdd itt: [Kezdes](#gyors-inditas-tldr)
...
A `cclawcore onboard` automatikusan lefut a telepites utan a munkater es szolgaltato konfiguralasakor.
...
cclawcore gateway                # alapertelmezett: 127.0.0.1:42617
cclawcore gateway --port 0       # veletlenszeru port (biztonsagi szilarditas)
...
Frissites? Futtasd a `cclawcore doctor` parancsot a frissites utan.
...
A CclawCore importalhatja az OpenClaw munkateret, memoriat es konfiguraciot:
...
Ez migralja a memoriabejegyzeseket, munkater fajlokat es konfiguraciot a `~/.openclaw/` konyvtarbol a `~/.cclawcore/` konyvtarba. A konfiguracio automatikusan JSON-bol TOML-ra konvertalodik.

## Biztonsagi alapertelmezesek (DM hozzaferes)

A CclawCore valos uzenetfeluletekkez csatlakozik. Kezeld a bejovo DM-eket nem megbizhato bemenetekkent.
...
- Jovahagy paranccsal: `cclawcore pairing approve <channel> <code>` (ezutan a felado felkerul egy lokalis engedelyezesi listara).
- A nyilvanos bejovo DM-ek kifejezett opt-in-t igenyelnek a `config.toml`-ban.
- Futtasd a `cclawcore doctor` parancsot a kockazatos vagy rosszul konfiguralt DM szabalyzatok feltarasahoz.
...
| ---------- | ----------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-02-19 | _Kritikus_ | **Nem** allunk kapcsolatban az `openagen/cclawcore`, `cclawcore.org` vagy `cclawcore.net` oldalakkal. A `cclawcore.org` es `cclawcore.net` domainek jelenleg az `openagen/cclawcore` fork-ra mutatnak, es az a domain/tarolo megszemelyesiti a hivatalos weboldalunkat/projektunket. | Ne bizz meg az ezekbol a forrasokbol szarmazo informaciokban, binarisokban, adomanygyujtesekben vagy kozlemenyekben. Kizarolag [ezt a tarolot](https://github.com/cclawcore-labs/cclawcore) es az ellenorzott kozossegi media fiokjainkat hasznald. |
...
### Miert valasztjak a csapatok a CclawCore-t
...
## Benchmark pillanatkep (CclawCore vs OpenClaw, Reprodukalhato)
...
> Megjegyzesek: A CclawCore eredmenyek release buildeken merve `/usr/bin/time -l` hasznalataval. Az OpenClaw Node.js futtatokornyezetet igenyel (tipikusan ~390MB memoria terheles), mig a NanoBot Python futtatokornyezetet. A PicoClaw es CclawCore statikus binarisok. A fenti RAM adatok futtatasi ideju memoriat mutatnak; a forditasi ideju kovetelmenyek magasabbak.
...
Minimalis `~/.cclawcore/config.toml`:
...
A CclawCore tamogatja az elofizetes-nativ hitelesitesi profilokat (tobb fiok, titkositva tarolva).

- Tarolo fajl: `~/.cclawcore/auth-profiles.json`
- Titkositasi kulcs: `~/.cclawcore/.secret_key`
...
Munkater gyoker: `~/.cclawcore/workspace/` (konfiguralhato a config-on keresztul).
...
Kepessegek: `~/.cclawcore/workspace/skills/<skill>/SKILL.md` vagy `SKILL.toml`.
...
```bash
# Munkater kezeles
cclawcore onboard              # Vezerelt beallitasi varazslo
cclawcore status               # Daemon/agent allapot megjelenites
cclawcore doctor               # Rendszer diagnosztika futtatasa

# Gateway + daemon
cclawcore gateway              # Gateway szerver inditasa (127.0.0.1:42617)
cclawcore daemon               # Teljes autonom futtatas inditasa

# Agens
cclawcore agent                # Interaktiv csevegesi mod
cclawcore agent -m "message"   # Egyszeri uzenet mod

# Szolgaltatas kezeles
cclawcore service install      # Telepites OS szolgaltataskent (launchd/systemd)
cclawcore service start|stop|restart|status

# Csatornak
cclawcore channel list         # Konfiguralt csatornak listazasa
cclawcore channel doctor       # Csatorna egeszseg ellenorzes
cclawcore channel bind-telegram 123456789

# Cron + utemezes
cclawcore cron list            # Utemezett feladatok listazasa
cclawcore cron add "*/5 * * * *" --prompt "Check system health"
cclawcore cron remove <id>

# Memoria
cclawcore memory list          # Memoriabejegyzesek listazasa
cclawcore memory get <key>     # Memoria lekerese
cclawcore memory stats         # Memoria statisztikak
...
# Hardver periferiak
cclawcore hardware discover    # Csatlakoztatott eszkozok keresese
cclawcore peripheral list      # Csatlakoztatott periferiak listazasa
cclawcore peripheral flash     # Firmware felirasa eszkozre
...
Vagy hagyd ki a fenti lepeseket es telepits mindent (rendszer fuggosegek, Rust, CclawCore) egyetlen paranccsal:
...
- Kovesd a [CclawCore Onboard](#gyors-inditas-tldr) szolgaltatast a vezerelt beallitashoz.
...
A CclawCore a smooth crab 🦀 szamara keszult, egy gyors es hatekony MI asszisztens. Epitette Argenis De La Rosa es a kozosseg.

- [cclawcorelabs.ai](https://cclawcorelabs.ai)
- [@cclawcorelabs](https://x.com/cclawcorelabs)

## Tamogasd a CclawCore-t

Ha a CclawCore segiti a munkadat es tamogatni szeretned a folyamatos fejlesztest, itt adomanyozhatsz:
...
Uj vagy a CclawCore-ban? Keresd a [`good first issue`](https://github.com/cclawcore-labs/cclawcore/issues?q=is%3Aissue+is%3Aopen+label%3A%22good+first+issue%22) cimkevel ellatott issue-kat — lasd a [Hozzajarulasi utmutatot](CONTRIBUTING.md#first-time-contributors) a kezdeshez. AI/vibe-coded PR-ok szivesen latottak! 🤖
...
- Uj `Skill` → `~/.cclawcore/workspace/skills/<name>/`
...
**Ez az egyetlen hivatalos CclawCore tarolo:**

> https://github.com/cclawcore-labs/cclawcore

Barmely mas tarolo, szervezet, domain vagy csomag, amely azt allitja, hogy "CclawCore" vagy kapcsolatot sugall a CclawCore Labs-szal, **jogosulatlan es nem all kapcsolatban ezzel a projekttel**. Az ismert jogosulatlan forkok a [TRADEMARK.md](docs/maintainers/trademark.md) fajlban lesznek felsorolva.

Ha megszemelyesitessel vagy vedjeggyel valo visszaelessel talalkozol, kerlek [nyiss egy issue-t](https://github.com/cclawcore-labs/cclawcore/issues).

---

## Licenc

A CclawCore kettos licenccel rendelkezik a maximalis nyitottsag es hozzajaruloi vedelem erdekeben:
...
A **CclawCore** nev es logo a CclawCore Labs vedjegyei. Ez a licenc nem ad engedelyt arra, hogy tamogatast vagy kapcsolatot sugalljanak. Lasd [TRADEMARK.md](docs/maintainers/trademark.md) a megengedett es tiltott hasznalati modokert.
...
**CclawCore** — Nulla terheles. Nulla kompromisszum. Telepites barhova. Csere barmire. 🦀
