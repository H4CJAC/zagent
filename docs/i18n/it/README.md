...
<h1 align="center">🦀 CclawCore — Assistente Personale IA</h1>
...
`cclawcore onboard` viene eseguito automaticamente dopo l'installazione per configurare il tuo workspace e provider.
...
cclawcore gateway                # predefinito: 127.0.0.1:42617
cclawcore gateway --port 0       # porta casuale (sicurezza rafforzata)
...
Aggiornamento? Esegui `cclawcore doctor` dopo l'aggiornamento.
...
CclawCore può importare il tuo workspace, memoria e configurazione da OpenClaw:
...
Questo migra le tue voci di memoria, i file del workspace e la configurazione da `~/.openclaw/` a `~/.cclawcore/`. La configurazione viene convertita da JSON a TOML automaticamente.

## Impostazioni di sicurezza predefinite (accesso DM)

CclawCore si connette a superfici di messaggistica reali. Tratta i DM in arrivo come input non attendibile.
...
- Approva con: `cclawcore pairing approve <channel> <code>` (il mittente viene quindi aggiunto a una allowlist locale).
- I DM pubblici in arrivo richiedono un'attivazione esplicita in `config.toml`.
- Esegui `cclawcore doctor` per individuare politiche DM rischiose o mal configurate.
...
| ---------- | ----------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-02-19 | _Critico_  | **Non siamo affiliati** con `openagen/cclawcore`, `cclawcore.org` o `cclawcore.net`. I domini `cclawcore.org` e `cclawcore.net` attualmente puntano al fork `openagen/cclawcore`, e quel dominio/repository stanno impersonando il nostro sito web/progetto ufficiale.                                                                                       | Non fidarti di informazioni, binari, raccolte fondi o annunci da quelle fonti. Usa solo [questo repository](https://github.com/cclawcore-labs/cclawcore) e i nostri account social verificati.                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
### Perché i team scelgono CclawCore
...
## Riepilogo benchmark (CclawCore vs OpenClaw, riproducibile)
...
> Note: I risultati di CclawCore sono misurati su build release usando `/usr/bin/time -l`. OpenClaw richiede il runtime Node.js (tipicamente ~390MB di overhead di memoria aggiuntivo), mentre NanoBot richiede il runtime Python. PicoClaw e CclawCore sono binari statici. I valori di RAM sopra sono memoria a runtime; i requisiti di compilazione sono superiori.
...
CclawCore supporta profili di autenticazione nativi tramite abbonamento (multi-account, crittografati a riposo).

- File di archiviazione: `~/.cclawcore/auth-profiles.json`
- Chiave di crittografia: `~/.cclawcore/.secret_key`
...
Root del workspace: `~/.cclawcore/workspace/` (configurabile tramite config).
...
Skill: `~/.cclawcore/workspace/skills/<skill>/SKILL.md` o `SKILL.toml`.
...
cclawcore onboard              # Procedura guidata di configurazione
cclawcore status               # Mostra stato del daemon/agente
cclawcore doctor               # Esegui diagnostica del sistema

# Gateway + daemon
cclawcore gateway              # Avvia server gateway (127.0.0.1:42617)
cclawcore daemon               # Avvia runtime autonomo completo

# Agente
cclawcore agent                # Modalità chat interattiva
cclawcore agent -m "message"   # Modalità messaggio singolo

# Gestione servizi
cclawcore service install      # Installa come servizio del SO (launchd/systemd)
cclawcore service start|stop|restart|status

# Canali
cclawcore channel list         # Elenca i canali configurati
cclawcore channel doctor       # Controlla la salute dei canali
cclawcore channel bind-telegram 123456789

# Cron + programmazione
cclawcore cron list            # Elenca i lavori programmati
cclawcore cron add "*/5 * * * *" --prompt "Check system health"
cclawcore cron remove <id>

# Memoria
cclawcore memory list          # Elenca le voci di memoria
cclawcore memory get <key>     # Recupera una memoria
cclawcore memory stats         # Statistiche della memoria
...
cclawcore hardware discover    # Scansiona i dispositivi connessi
cclawcore peripheral list      # Elenca le periferiche connesse
cclawcore peripheral flash     # Flash del firmware sul dispositivo
...
Oppure salta i passaggi precedenti e installa tutto (dipendenze di sistema, Rust, CclawCore) con un solo comando:
...
- Segui [CclawCore Onboard](#avvio-rapido) per una configurazione guidata.
...
CclawCore è stato costruito per il granchio liscio 🦀, un assistente IA veloce ed efficiente. Costruito da Argenis De La Rosa e la comunità.
...
Se CclawCore ti aiuta nel lavoro e vuoi supportare lo sviluppo continuo, puoi donare qui:
...
Nuovo su CclawCore? Cerca le issue etichettate [`good first issue`](https://github.com/cclawcore-labs/cclawcore/issues?q=is%3Aissue+is%3Aopen+label%3A%22good+first+issue%22) — consulta la nostra [Guida alla contribuzione](CONTRIBUTING.md#first-time-contributors) per sapere come iniziare. PR con IA/vibe-coded sono benvenuti! 🤖
...
- Nuovo `Skill` → `~/.cclawcore/workspace/skills/<name>/`
...
**Questo è l'unico repository ufficiale di CclawCore:**

> https://github.com/cclawcore-labs/cclawcore

Qualsiasi altro repository, organizzazione, dominio o pacchetto che affermi di essere "CclawCore" o implichi un'affiliazione con CclawCore Labs **non è autorizzato e non è affiliato a questo progetto**. I fork non autorizzati conosciuti saranno elencati in [TRADEMARK.md](docs/maintainers/trademark.md).

Se incontri impersonificazione o uso improprio del marchio, per favore [apri una issue](https://github.com/cclawcore-labs/cclawcore/issues).

---

## Licenza

CclawCore ha doppia licenza per massima apertura e protezione dei contributori:
...
Il nome e il logo di **CclawCore** sono marchi di CclawCore Labs. Questa licenza non concede il permesso di usarli per implicare approvazione o affiliazione. Vedi [TRADEMARK.md](docs/maintainers/trademark.md) per gli usi consentiti e proibiti.
...
**CclawCore** — Zero overhead. Zero compromessi. Distribuisci ovunque. Scambia qualsiasi cosa. 🦀
