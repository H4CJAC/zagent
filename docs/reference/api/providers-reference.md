...
3. Generic fallback env vars: `CCLAWCORE_API_KEY` then `API_KEY`
...
CclawCore supports automatic failover to alternative providers when the primary encounters:
...
When a provider returns 429 (rate limit), CclawCore:
...
- After multimodal normalization, CclawCore sends image payloads through Ollama's native `messages[].images` field.
- If a non-vision provider is selected, CclawCore returns a structured capability error instead of silently ignoring images.
...
- CclawCore normalizes a trailing `/api` in `api_url` automatically.
...
- Model discovery: `cclawcore models refresh --provider llamacpp`
...
- Model discovery: `cclawcore models refresh --provider sglang`
...
- Model discovery: `cclawcore models refresh --provider vllm`
...
- Model discovery: `cclawcore models refresh --provider osaurus`
...
- Model discovery: `cclawcore models refresh --provider nvidia`
