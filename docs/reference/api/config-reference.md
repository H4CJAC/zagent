# CclawCore Config Reference (Operator-Oriented)
...
1. `CCLAWCORE_WORKSPACE` override (if set)
2. persisted `~/.cclawcore/active_workspace.toml` marker (if present)
3. default `~/.cclawcore/config.toml`

CclawCore logs the resolved config on startup at `INFO` level:

- `Config loaded` with fields: `path`, `workspace`, `source`, `initialized`

Schema export command:

- `cclawcore config schema` (prints JSON Schema draft 2020-12 to stdout)
...
| `otel_service_name` | `cclawcore` | Service name emitted to OTLP collector |
...
- Query runtime traces with:
  - `cclawcore doctor traces --limit 20`
  - `cclawcore doctor traces --event tool_call_result --contains \"error\"`
  - `cclawcore doctor traces --id <trace-id>`
...
1. `CCLAWCORE_PROVIDER` (explicit override, always wins when non-empty)
...
- Use `CCLAWCORE_PROVIDER` when you intentionally want runtime env to override a non-default configured provider.
...
- Each fallback provider resolves credentials independently using the standard resolution order: explicit config → provider-specific env var → `CCLAWCORE_API_KEY` → `API_KEY`.
...
- `api_keys` supplies additional API keys that CclawCore rotates through on `429` (rate limit) responses. The primary `api_key` (set globally or per-channel) is tried first.
...
- When `enabled = true` and no OTP secret exists, CclawCore generates one and prints an enrollment URI once.
...
| `state_file` | `~/.cclawcore/estop-state.json` | Persistent estop state path |
...
- Use CLI command `cclawcore estop` to engage and `cclawcore estop resume` to clear levels.
...
- Security-first default: CclawCore does **not** clone or sync `open-skills` unless `open_skills_enabled = true`.
- Environment overrides:
  - `CCLAWCORE_OPEN_SKILLS_ENABLED` accepts `1/0`, `true/false`, `yes/no`, `on/off`.
  - `CCLAWCORE_OPEN_SKILLS_DIR` overrides the repository path when non-empty.
  - `CCLAWCORE_SKILLS_PROMPT_MODE` accepts `full` or `compact`.
- Precedence for enable flag: `CCLAWCORE_OPEN_SKILLS_ENABLED` → `skills.open_skills_enabled` in `config.toml` → default `false`.
...
- Skill loading and `cclawcore skills install` both apply a static security audit. Skills that contain symlinks, script-like files, high-risk shell payload snippets, or unsafe markdown link traversal are rejected.
...
- CclawCore requests Composio v3 tools with `toolkit_versions=latest` and executes tools with `version="latest"` to avoid stale default tool revisions.
...
| `path_prefix` | _(none)_ | URL path prefix for reverse-proxy deployments (e.g. `"/cclawcore"`) |

When deploying behind a reverse proxy that maps CclawCore to a sub-path,
set `path_prefix` to that sub-path (e.g. `"/cclawcore"`). All gateway
...
3. Validate with `cclawcore doctor` before restart/rollout.
...
- While `cclawcore channel start` is running, updates to `default_provider`, `default_model`, `default_temperature`, `api_key`, `api_url`, and `reliability.*` are hot-applied from `config.toml` on the next inbound message.
...
- `CCLAWCORE_LINQ_SIGNING_SECRET` overrides `signing_secret` when set.
...
| `bot_name` | Optional | Display name of the bot in Nextcloud Talk (e.g. `"cclawcore"`). Used to filter out the bot's own messages and prevent feedback loops. |

Notes:

- Webhook endpoint is `POST /nextcloud-talk`.
- `CCLAWCORE_NEXTCLOUD_TALK_WEBHOOK_SECRET` overrides `webhook_secret` when set.
