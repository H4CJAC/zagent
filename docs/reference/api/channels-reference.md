...
This document is the canonical reference for channel configuration in CclawCore.
...
6. **Stale runtime state**: config changed but `cclawcore daemon` was not restarted.

---

## 1. Configuration Namespace

All channel settings live under `channels_config` in `~/.cclawcore/config.toml`.
...
When running `cclawcore channel start` (or daemon mode), Telegram and Discord now support sender-scoped runtime switching:
...
- Model cache previews come from `cclawcore models refresh --provider <ID>`.
- These are runtime chat commands, not CLI subcommands.

## Inbound Image Marker Protocol

CclawCore supports multimodal input through inline message markers:
...
If `[channels_config.matrix]`, `[channels_config.lark]`, or `[channels_config.feishu]` is present but the corresponding feature is not compiled in, `cclawcore channel list`, `cclawcore channel doctor`, and `cclawcore channel start` will report that the channel is intentionally skipped for this build.
...
access_token = "syt_..."
user_id = "@cclawcore:matrix.example.com"   # optional, recommended for E2EE
...
CclawCore supports two WhatsApp backends:
...
```toml
[channels_config.whatsapp]
session_path = "~/.cclawcore/state/whatsapp-web/session.db"
...
port = 6697
nickname = "cclawcore-bot"
username = "cclawcore"              # optional
channels = ["#cclawcore"]
allowed_users = ["*"]
...
allowed_users = ["*"]
# bot_name = "cclawcore"  # display name of the bot; filters own messages to prevent feedback loops
```
...
- `CCLAWCORE_NEXTCLOUD_TALK_WEBHOOK_SECRET` overrides config secret.
...
- `CCLAWCORE_LINQ_SIGNING_SECRET` overrides config secret.
...
5. Restart `cclawcore daemon` after config changes.
...
```bash
RUST_LOG=info cclawcore daemon 2>&1 | tee /tmp/cclawcore.log
```

Then filter channel/gateway events:

```bash
rg -n "Matrix|Telegram|Discord|Slack|Mattermost|Signal|WhatsApp|Email|IRC|Lark|DingTalk|QQ|iMessage|Nostr|Webhook|Channel" /tmp/cclawcore.log
```
