...
- CLI `cclawcore sop` currently manages definitions only: `list`, `validate`, `show`.
...
2. Create a SOP directory, for example:
   ...
   ```text
   ~/.cclawcore/workspace/sops/deploy-prod/SOP.toml
   ~/.cclawcore/workspace/sops/deploy-prod/SOP.md
   ```
...
3. Validate and inspect definitions:
   ...
   ```bash
   cclawcore sop list
   cclawcore sop validate
   cclawcore sop show deploy-prod
   ```
