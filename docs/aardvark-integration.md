...
The rest of CclawCore speaks a single language: `ZcCommand` → `ZcResponse`.
...
DatasheetTool.call(args)
  ...
  → "download": fetch PDF from args["url"] → save to ~/.cclawcore/hardware/datasheets/
...
```
 SDK FILES          aardvark-sys            CclawCore core
 (vendor/)          (crates/)               (src/)
─────────────────────────────────────────────────────────────────
