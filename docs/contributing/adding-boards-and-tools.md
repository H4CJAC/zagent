# Adding Boards and Tools — CclawCore Hardware Guide

This guide explains how to add new hardware boards and custom tools to CclawCore.

## Quick Start: Add a Board via CLI

```bash
# Add a board (updates ~/.cclawcore/config.toml)
cclawcore peripheral add nucleo-f401re /dev/ttyACM0
cclawcore peripheral add arduino-uno /dev/cu.usbmodem12345
cclawcore peripheral add rpi-gpio native   # for Raspberry Pi GPIO (Linux)
...
With the `rag-pdf` feature, CclawCore can index PDF files:
...
2. **Add to config** — `cclawcore peripheral add my-board /dev/ttyUSB0`
...
| Command | Description |
|---------|-------------|
| `cclawcore peripheral list` | List configured boards |
| `cclawcore peripheral add <board> <path>` | Add board (writes config) |
| `cclawcore peripheral flash` | Flash Arduino firmware |
| `cclawcore peripheral flash-nucleo` | Flash Nucleo firmware |
| `cclawcore hardware discover` | List USB devices |
| `cclawcore hardware info` | Chip info via probe-rs |
