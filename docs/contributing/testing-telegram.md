...
```bash
# Check config
cat ~/.cclawcore/config.toml | grep -A 5 telegram
...
| Health check | <5s | `time cclawcore channel doctor` |
...
5. **Monitor logs** for any issues:
   ...
   cclawcore daemon
   # or
   RUST_LOG=info cclawcore channel start
   ```
...
- Issues: https://github.com/cclawcore-labs/cclawcore/issues
