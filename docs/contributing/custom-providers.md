...
CclawCore supports custom API endpoints for both OpenAI-compatible and Anthropic-compatible providers.
...
# or: export CCLAWCORE_API_KEY="your-api-key"
cclawcore agent
```

## llama.cpp Server (Recommended Local Setup)

CclawCore includes a first-class local provider for `llama-server`:
...
```bash
cclawcore models refresh --provider llamacpp
cclawcore agent -m "hello"
```

You do not need to export `CCLAWCORE_API_KEY=dummy` for this flow.

## SGLang Server

CclawCore includes a first-class local provider for [SGLang](https://github.com/sgl-project/sglang):
...
```bash
cclawcore models refresh --provider sglang
cclawcore agent -m "hello"
```
...
CclawCore includes a first-class local provider for [vLLM](https://docs.vllm.ai/):
...
```bash
cclawcore models refresh --provider vllm
cclawcore agent -m "hello"
```
...
# Single message test
cclawcore agent -m "test message"
```
