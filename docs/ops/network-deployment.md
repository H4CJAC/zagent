# Network Deployment — CclawCore on Raspberry Pi and Local Network

This document covers deploying CclawCore on a Raspberry Pi or other host on your local network, with Telegram and optional webhook channels.
...
|------|----------------------|----------|
| **Telegram polling** | No | CclawCore polls Telegram API; works from anywhere |
| **Matrix sync (including E2EE)** | No | CclawCore syncs via Matrix client API; no inbound webhook required |
...
**Key:** Telegram, Discord, Slack, and Nostr use **outbound connections** — CclawCore connects to external servers/relays. No port forwarding or public IP required.

---

## 2. CclawCore on Raspberry Pi
...
- Telegram channel works: CclawCore polls Telegram API (outbound)
...
```bash
cclawcore daemon --host 0.0.0.0 --port 42617
```
...
2. Start a tunnel:
   ...
   ```
   Or use `cclawcore tunnel` (see tunnel docs).

3. CclawCore will refuse `0.0.0.0` unless `allow_public_bind = true` or a tunnel is active.
...
- CclawCore calls `https://api.telegram.org/bot{token}/getUpdates`
...
Run `cclawcore daemon` — Telegram channel starts automatically.
...
- Keep one runtime instance for the same token (recommended: `cclawcore daemon` service).
...
- [ ] Run `cclawcore daemon --host 127.0.0.1 --port 42617` (Telegram works without 0.0.0.0)
...
CclawCore supports OpenRC for Alpine Linux and other distributions using the OpenRC init system. OpenRC services run **system-wide** and require root/sudo.

### 7.1 Prerequisites

- Alpine Linux (or another OpenRC-based distro)
- Root or sudo access
- A dedicated `cclawcore` system user (created during install)

### 7.2 Install Service

```bash
# Install service (OpenRC is auto-detected on Alpine)
sudo cclawcore service install
```

This creates:
- Init script: `/etc/init.d/cclawcore`
- Config directory: `/etc/cclawcore/`
- Log directory: `/var/log/cclawcore/`

### 7.3 Configuration

Manual config copy is usually not required.

`sudo cclawcore service install` automatically prepares `/etc/cclawcore`, migrates existing runtime state from your user setup when available, and sets ownership/permissions for the `cclawcore` service user.

If no prior runtime state is available to migrate, create `/etc/cclawcore/config.toml` before starting the service.

### 7.4 Enable and Start

```bash
# Add to default runlevel
sudo rc-update add cclawcore default

# Start the service
sudo rc-service cclawcore start

# Check status
sudo rc-service cclawcore status
```
...
| Command | Description |
|---------|-------------|
| `sudo rc-service cclawcore start` | Start the daemon |
| `sudo rc-service cclawcore stop` | Stop the daemon |
| `sudo rc-service cclawcore status` | Check service status |
| `sudo rc-service cclawcore restart` | Restart the daemon |
| `sudo cclawcore service status` | CclawCore status wrapper (uses `/etc/cclawcore` config) |
...
| Log | Path |
|-----|------|
| Access/stdout | `/var/log/cclawcore/access.log` |
| Errors/stderr | `/var/log/cclawcore/error.log` |

View logs:

```bash
sudo tail -f /var/log/cclawcore/error.log
```
...
sudo rc-service cclawcore stop
sudo rc-update del cclawcore default

# Remove init script
sudo cclawcore service uninstall
```
...
- The service runs as the `cclawcore:cclawcore` user (least privilege)
- Config must be at `/etc/cclawcore/config.toml` (explicit path in init script)
- If the `cclawcore` user does not exist, install will fail with instructions to create it

### 7.9 Checklist: Alpine/OpenRC Deployment

- [ ] Install: `sudo cclawcore service install`
- [ ] Enable: `sudo rc-update add cclawcore default`
- [ ] Start: `sudo rc-service cclawcore start`
- [ ] Verify: `sudo rc-service cclawcore status`
- [ ] Check logs: `/var/log/cclawcore/error.log`
