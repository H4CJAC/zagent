...
impl TelegramChannel {
    ...
    async fn load_config_without_env() -> anyhow::Result<Config> {
        ...
        config.workspace_dir = cclawcore_dir.join("workspace");
        Ok(config)
    }
                                ...
                                Ok(()) => {
                                    let _ = self
                                        .send(&SendMessage::new(
                                            "✅ Telegram account bound successfully. You can talk to CclawCore now.",
                                            &chat_id,
                                        ))
                                        .await;
                                    tracing::info!(
...
Ensure only one `cclawcore` process is using this bot token."
                    );
...
mod tests {
    ...
    fn telegram_extract_bind_code_supports_bot_mention() {
        assert_eq!(
            TelegramChannel::extract_bind_code("/bind@cclawcore_bot 654321"),
            Some("654321")
        );
    }
