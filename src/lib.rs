...
Examples:
  cclawcore gateway start              # use config defaults
  cclawcore gateway start -p 8080      # listen on port 8080
  cclawcore gateway start --host 0.0.0.0   # requires [gateway].allow_public_bind=true or a tunnel
  cclawcore gateway start -p 0         # random available port")]
    Start {
...
Examples:
  cclawcore gateway restart            # restart with config defaults
  cclawcore gateway restart -p 8080    # restart on port 8080")]
    Restart {
        /// Port to listen on (use 0 for random available port); defaults to config gateway.port
        #[arg(short, long)]
        port: Option<u16>,
...
Examples:
  cclawcore gateway get-paircode       # show current pairing code
  cclawcore gateway get-paircode --new # generate a new pairing code")]
    GetPaircode {
...
Examples:
  ...
  cclawcore channel add discord '{\"bot_token\":\"...\",\"name\":\"my-discord\"}'")]
    Add {
...
Examples:
  cclawcore channel send 'Someone is near your device.' --channel-id telegram --recipient 123456789
  cclawcore channel send 'Build succeeded!' --channel-id discord --recipient 987654321")]
    Send {
...
pub enum MigrateCommands {
    /// Import memory from an `OpenClaw` workspace into this `CclawCore` workspace
    Openclaw {
...
Examples:
  cclawcore cron add-at 2025-01-15T14:00:00Z 'Send reminder'
  cclawcore cron add-at 2025-12-31T23:59:00Z 'Happy New Year!'")]
    AddAt {
...
Examples:
  cclawcore cron add-every 60000 'Ping heartbeat'     # every minute
  cclawcore cron add-every 3600000 'Hourly report'    # every hour")]
    AddEvery {
...
Examples:
  cclawcore cron once 30m 'Run backup in 30 minutes'
  cclawcore cron once 2h 'Follow up on deployment'
  cclawcore cron once 1d 'Daily check'")]
    Once {
        /// Delay duration
        delay: String,
        /// Treat the argument as an agent prompt instead of a shell command
...
Examples:
  cclawcore cron update <task-id> --expression '0 8 * * *'
  cclawcore cron update <task-id> --tz Europe/London --name 'Morning check'
  cclawcore cron update <task-id> --command 'Updated message'")]
    Update {
        /// Task ID
        id: String,
        /// New cron expression
...
Examples:
  ...
  cclawcore hardware introspect COM3")]
    Introspect {
...
Examples:
  ...
  cclawcore peripheral add esp32 /dev/ttyUSB0")]
    Add {
    ...
    /// Flash CclawCore firmware to Arduino (creates .ino, installs arduino-cli if needed, uploads)
    #[command(long_about = "\
Flash CclawCore firmware to an Arduino board.

Generates the .ino sketch, installs arduino-cli if it is not \
already available, compiles, and uploads the firmware.

Examples:
  cclawcore peripheral flash
  cclawcore peripheral flash --port /dev/cu.usbmodem12345
  cclawcore peripheral flash -p COM3")]
    Flash {
    ...
    /// Flash CclawCore firmware to Nucleo-F401RE (builds + probe-rs run)
    FlashNucleo,
}
