...
const CODEX_RESPONSES_URL_ENV: &str = "CCLAWCORE_CODEX_RESPONSES_URL";
const CODEX_BASE_URL_ENV: &str = "CCLAWCORE_CODEX_BASE_URL";
const DEFAULT_CODEX_INSTRUCTIONS: &str =
    "You are CclawCore, a concise and helpful coding assistant.";
...
impl OpenAiCodexProvider {
    pub fn new(
        ...
        let state_dir = options
            .cclawcore_dir
            .clone()
            .unwrap_or_else(default_cclawcore_dir);
...
fn default_cclawcore_dir() -> PathBuf {
...
fn resolve_reasoning_effort(model_id: &str, configured: Option<&str>) -> String {
    let raw = configured
        ...
        .or_else(|| std::env::var("CCLAWCORE_CODEX_REASONING_EFFORT").ok())
        .and_then(|value| first_nonempty(Some(&value)))
    ...
    async fn send_responses_request(
        ...
        let access_token = if use_gateway_api_key_auth {
            ...
            Some(oauth_access_token.ok_or_else(|| {
                anyhow::anyhow!(
                    "OpenAI Codex auth profile not found. Run `cclawcore auth login --provider openai-codex`."
                )
            })?)
        };
        let account_id = if use_gateway_api_key_auth {
            ...
            Some(account_id.ok_or_else(|| {
                anyhow::anyhow!(
                    "OpenAI Codex account id not found in auth profile/token. Run `cclawcore auth login --provider openai-codex` again."
                )
            })?)
        };
...
mod tests {
    ...
    fn default_state_dir_is_non_empty() {
        let path = default_cclawcore_dir();
    ...
    fn resolve_reasoning_effort_prefers_configured_override() {
        let _lock = env_lock();
        let _guard = EnvGuard::set("CCLAWCORE_CODEX_REASONING_EFFORT", Some("low"));
        assert_eq!(
    ...
    fn resolve_reasoning_effort_uses_legacy_env_when_unconfigured() {
        let _lock = env_lock();
        let _guard = EnvGuard::set("CCLAWCORE_CODEX_REASONING_EFFORT", Some("minimal"));
        assert_eq!(
