...
//!
//! Wire protocol (CclawCore serial JSON):
//! ```text
...
/// Default baud rate for CclawCore serial devices.
...
/// Serial transport for CclawCore hardware devices.
///
...
impl HardwareSerialTransport {
    ...
    /// Attempt a ping handshake to verify CclawCore firmware is running.
    ///
    /// Opens the port, sends `{"cmd":"ping","params":{}}`, waits up to
    /// `PING_TIMEOUT_MS` for a response with `data.firmware == "cclawcore"`.
    ///
    /// Returns `true` if a CclawCore device responds, `false` otherwise.
    ...
    pub async fn ping_handshake(&self) -> bool {
        ...
        match result {
            Ok(Ok(resp)) => {
                // Accept if firmware field is "cclawcore" (in data or top-level)
                resp.ok
                    && resp
                        .data
                        .get("firmware")
