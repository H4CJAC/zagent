...
//!    `~/.cclawcore/firmware/pico/` if it isn't there yet.
...
const PICO_UF2: &[u8] = include_bytes!("../../firmware/pico/cclawcore-pico.uf2");
...
/// Ensure `~/.cclawcore/firmware/pico/` exists and contains the bundled assets.
///
...
pub fn ensure_firmware_dir() -> Result<PathBuf> {
    ...
    let uf2_path = firmware_dir.join("cclawcore-pico.uf2");
    if !uf2_path.exists() {
        if PICO_UF2.len() < 8 || PICO_UF2[..4] != UF2_MAGIC1 {
            bail!(
                "Bundled UF2 is a placeholder — download the real MicroPython UF2 from \
                 ...
                 src/firmware/pico/cclawcore-pico.uf2, then rebuild CclawCore."
            );
        }
...
pub async fn flash_uf2(mount_point: &Path, firmware_dir: &Path) -> Result<()> {
    let uf2_src = firmware_dir.join("cclawcore-pico.uf2");
    ...
    if data.len() < 8 || data[..4] != UF2_MAGIC1 {
        bail!(
            "UF2 at {} does not look like a valid UF2 file (magic mismatch). \
             ...
             the existing file so CclawCore can re-extract it.",
            uf2_src.display()
        );
    }
    ...
    bail!(
        "All copy methods failed. Run this command manually, then restart CclawCore:\n\
...
mod tests {
    ...
    fn ensure_firmware_dir_creates_directory() {
        // This test verifies ensure_firmware_dir creates the ~/.cclawcore/firmware/pico/ path.
    ...
    async fn flash_uf2_rejects_invalid_magic() {
        ...
        std::fs::write(firmware_dir.join("cclawcore-pico.uf2"), b"NOT_A_UF2_FILE").unwrap();
    ...
    async fn flash_uf2_rejects_too_small_file() {
        ...
        std::fs::write(firmware_dir.join("cclawcore-pico.uf2"), b"tiny").unwrap();
