...
// in `~/.cclawcore/.secret_key` with restrictive file permissions (0600). The
...
pub struct SecretStore {
    /// Path to the key file (`~/.cclawcore/.secret_key`)
    key_path: PathBuf,
...
impl SecretStore {
    ...
    pub fn new(cclawcore_dir: &Path, enabled: bool) -> Self {
        Self {
            key_path: cclawcore_dir.join(".secret_key"),
            enabled,
        }
    }
