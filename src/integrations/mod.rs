...
fn show_integration_info(config: &Config, name: &str) -> Result<()> {
    ...
    let Some(entry) = entries.iter().find(|e| e.name.to_lowercase() == name_lower) else {
        anyhow::bail!(
            "Unknown integration: {name}. Check README for supported integrations or run `cclawcore onboard` to configure channels/providers."
        );
    };
    ...
    match entry.name {
        "Telegram" => {
            ...
            println!("    3. Run: cclawcore onboard --channels-only");
            println!("    4. Start: cclawcore channel start");
        }
        "Discord" => {
            println!("  Setup:");
            ...
            println!("    4. Run: cclawcore onboard --channels-only");
        }
        "Slack" => {
            println!("  Setup:");
        ...
        "OpenRouter" => {
            ...
            println!("    2. Run: cclawcore onboard");
        ...
        "Browser" => {
            println!("  Built-in:");
            println!("    CclawCore can control Chrome/Chromium for web tasks.");
            println!("    Uses headless browser automation.");
        }
        "Cron" => {
            println!("  Built-in:");
            println!("    Schedule tasks in ~/.cclawcore/workspace/cron/");
            println!("    Run: cclawcore cron list");
        }
        "Weather" => {
            println!("  Built-in:");
        ...
        "Webhooks" => {
            ...
            println!("    Run: cclawcore gateway");
        }
        _ => {
            if status == IntegrationStatus::ComingSoon {
                println!("  This integration is planned. Stay tuned!");
                println!("  Track progress: https://github.com/cclawcore-labs/cclawcore");
            }
        }
    }
