...
//!
//! CclawCore's agent loop calls [`crate::hardware::boot`] on **every** request,
//! which re-reads `~/.cclawcore/hardware/` from disk.  Writing to those files
...
/// Return `~/.cclawcore/hardware/` or an error string.
fn hardware_dir() -> Result<PathBuf, String> {
    directories::BaseDirs::new()
        .map(|b| b.home_dir().join(".cclawcore").join("hardware"))
...
/// `POST /api/hardware/pin` — register a single GPIO pin assignment.
///
/// Appends one line to `~/.cclawcore/hardware/devices/<device>.md`:
/// ```text
