...
const OPEN_SKILLS_SYNC_MARKER: &str = ".cclawcore-open-skills-sync";
...
/// Skills live in `~/.cclawcore/workspace/skills/<name>/SKILL.md`
    ...
    if scripts_blocked && !allow_scripts {
        ...
        eprintln!(
            "warning: skill '{}' was skipped because it contains script files. \
             Set `skills.allow_scripts = true` in your cclawcore config to enable it.",
            path.file_name()
...
fn open_skills_enabled(config_open_skills_enabled: Option<bool>) -> bool {
    let env_override = std::env::var("CCLAWCORE_OPEN_SKILLS_ENABLED").ok();
...
fn resolve_open_skills_dir(config_open_skills_dir: Option<&str>) -> Option<PathBuf> {
    let env_dir = std::env::var("CCLAWCORE_OPEN_SKILLS_DIR").ok();
    ...
    match command {
        crate::SkillCommands::List => {
            ...
            if skills.is_empty() {
                ...
                println!();
                println!("  Create one: mkdir -p ~/.cclawcore/workspace/skills/my-skill");
                println!(
                    "              echo '# My Skill' > ~/.cclawcore/workspace/skills/my-skill/SKILL.md"
                );
                println!();
                println!("  Or install: cclawcore skills install <source>");
            } else {
...
"#,
    ...
    #[test]
    fn skills_dir_path() {
        let base = std::path::Path::new("/home/user/.cclawcore");
        let dir = skills_dir(base);
        assert_eq!(dir, PathBuf::from("/home/user/.cclawcore/skills"));
    }
    ...
    fn load_skills_with_config_reads_open_skills_dir_without_network() {
        ...
        let _enabled_guard = EnvVarGuard::unset("CCLAWCORE_OPEN_SKILLS_ENABLED");
        let _dir_guard = EnvVarGuard::unset("CCLAWCORE_OPEN_SKILLS_DIR");
