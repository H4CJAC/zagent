...
/// - `X-CclawCore-Timestamp` — unix epoch seconds
/// - `X-CclawCore-Nonce` — random UUID v4
/// - `X-CclawCore-Signature` — HMAC-SHA256 hex digest
///
...
impl NodeTransport {
    ...
    pub async fn send(
        ...
        let resp = self
            .http
            .post(&url)
            .header("X-CclawCore-Timestamp", timestamp.to_string())
            .header("X-CclawCore-Nonce", &nonce)
            .header("X-CclawCore-Signature", &signature)
