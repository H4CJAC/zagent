...
impl WebSearchTool {
    ...
    fn reload_brave_api_key(&self) -> anyhow::Result<String> {
        ...
        if crate::security::SecretStore::is_encrypted(&raw_key) {
            let cclawcore_dir = self.config_path.parent().unwrap_or_else(|| Path::new("."));
            let store = crate::security::SecretStore::new(cclawcore_dir, self.secrets_encrypt);
