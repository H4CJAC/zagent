//! Wraps a discovered MCP tool as a cclawcore [`Tool`] so it is dispatched
...
/// A cclawcore [`Tool`] backed by an MCP server tool.
///
...
impl Tool for McpToolWrapper {
    ...
    async fn execute(&self, args: serde_json::Value) -> anyhow::Result<ToolResult> {
        ...
        // CclawCore's security model injects `approved: bool` into built-in tool
...
mod tests {
    ...
    // CclawCore's security model injects `approved: bool` into built-in tool args.
