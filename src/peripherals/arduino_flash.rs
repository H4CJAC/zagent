//! Flash CclawCore Arduino firmware via arduino-cli.
//!
...
/// CclawCore Arduino Uno base firmware (capabilities, gpio_read, gpio_write).
...
/// Flash CclawCore firmware to Arduino at the given port.
pub fn flash_arduino_firmware(port: &str) -> Result<()> {
    ensure_arduino_cli()?;
    ensure_avr_core()?;

    let temp_dir = std::env::temp_dir().join(format!("cclawcore_flash_{}", uuid::Uuid::new_v4()));
    ...
    // Compile
    println!("Compiling CclawCore Arduino firmware...");
    ...
    println!("CclawCore firmware flashed successfully.");
