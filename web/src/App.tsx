...
> {
  ...
  componentDidCatch(error: Error, info: ErrorInfo) {
    console.error('[CclawCore] Render error:', error, info.componentStack);
  }
    ...
    <div className="min-h-screen flex items-center justify-center" style={{ background: 'var(--pc-bg-base)' }}>
      ...
      <div className="relative surface-panel p-8 w-full max-w-md animate-fade-in-scale">
        ...
        <div className="text-center mb-8">
          <img
            src={`${basePath}/_app/cclawcore-trans.png`}
            alt="CclawCore"
            className="h-20 w-20 rounded-2xl object-cover mx-auto mb-4 animate-float"
            onError={(e) => { e.currentTarget.style.display = 'none'; }}
          />
          <h1 className="text-2xl font-bold mb-2 text-gradient-blue">CclawCore</h1>
...
function AppContent() {
  ...
  useEffect(() => {
    ...
    window.addEventListener('cclawcore-unauthorized', handler);
    return () => window.removeEventListener('cclawcore-unauthorized', handler);
  }, [logout]);
