...
2. **GitHub**: Use [GitHub Security Advisories](https://github.com/cclawcore-labs/cclawcore/security/advisories/new)
...
CclawCore implements defense-in-depth security:
...
CclawCore Docker images follow CIS Docker Benchmark best practices:
...
docker build -t cclawcore .
docker inspect --format='{{.Config.User}}' cclawcore
# Expected: 65534:65534

# Run with read-only filesystem (production hardening)
docker run --read-only -v /path/to/workspace:/workspace cclawcore gateway
```
