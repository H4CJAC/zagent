...
# ── Step 1: Build ────────────────────────────────────────────────────
echo -e "${BOLD}[1/3] Building cclawcore (${PROFILE})...${RESET}"
if [[ "$PROFILE" == "release" ]]; then
  cargo build --release 2>&1
  BIN="$REPO_ROOT/target/release/cclawcore"
else
  cargo build 2>&1
  BIN="$REPO_ROOT/target/debug/cclawcore"
fi
...
echo "  [ ] Welcome screen renders with CCLAWCORE banner"
