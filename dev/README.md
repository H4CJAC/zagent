# CclawCore Development Environment

A fully containerized development sandbox for CclawCore agents. This environment allows you to develop, test, and debug the agent in isolation without modifying your host system.
...
### 2. Enter Agent Container (`cclawcore-dev`)

```bash
./dev/cli.sh agent
```

Use this to run `cclawcore` CLI commands manually, debug the binary, or check logs internally.
...
- **Agent**: `/cclawcore-data/workspace`
...
The agent configuration lives in `target/.cclawcore` (mounted to `/cclawcore-data/.cclawcore`), so settings persist across container rebuilds.
...
**Note:** This removes `target/.cclawcore` (config/DB) but leaves the `playground/` directory intact. To fully wipe workspace data, manually delete `playground/`.
...
- Rust compilation, tests, and audit/deny tools run in `cclawcore-local-ci` container.
...
- The root `Dockerfile` also caches Rust `target/` (`id=cclawcore-target`) to speed repeat local image builds.
