...
ASSET_URL="https://github.com/cclawcore-labs/cclawcore/releases/download/${TAG}/${ASSET_NAME}"
TEMP_DIR="$(mktemp -d -t cclawcore-termux-test-XXXXXX)"
...
# --- Test 5: Contains cclawcore binary ---
info "Checking archive contents"
CONTENTS=$(tar -tzf "$TEMP_DIR/$ASSET_NAME")
if echo "$CONTENTS" | grep -q "^cclawcore$"; then
  pass "Archive contains 'cclawcore' binary"
else
  fail "Archive does not contain 'cclawcore' binary"
  echo "Contents:"
  echo "$CONTENTS" | sed 's/^/  /'
fi
...
CHECKSUMS_URL="https://github.com/cclawcore-labs/cclawcore/releases/download/${TAG}/SHA256SUMS"
