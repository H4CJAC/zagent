# Marketplace Templates for CclawCore

This directory contains draft templates and CI/CD workflows for listing CclawCore
on self-hosted PaaS platforms.

## Platforms

### Coolify (coollabsio/coolify)
- Template: `coolify/cclawcore.yaml` -> goes to `templates/compose/cclawcore.yaml` in their repo
- Logo: needs `cclawcore.svg` in their `svgs/` directory
- PR target branch: `next` (CRITICAL — they close PRs to other branches)

### Dokploy (Dokploy/templates)
- Blueprint: `dokploy/blueprints/cclawcore/` -> goes to `blueprints/cclawcore/` in their repo
- Meta entry: `dokploy/meta-entry.json` -> merge into root `meta.json`
- Logo: needs `cclawcore.svg` in the blueprint folder
- PR target branch: `main`
- IMPORTANT: Dokploy requires pinned image versions (no `latest` tag)

### EasyPanel (easypanel-io/templates)
- Template: `easypanel/` -> goes to `templates/cclawcore/` in their repo
...
- [ ] **Copy the SVG logo** from `apps/tauri/icons/icon.svg` to `.github/assets/cclawcore.svg`:
      ```bash
      cp apps/tauri/icons/icon.svg .github/assets/cclawcore.svg
      git add .github/assets/cclawcore.svg && git commit -m "chore: add SVG logo for marketplace templates"
      ```
- [ ] **Fork all three upstream repos** into the `cclawcore-labs` org:
      - Fork `coollabsio/coolify` -> `cclawcore-labs/coolify`
      - Fork `Dokploy/templates` -> `cclawcore-labs/templates`
      - Fork `easypanel-io/templates` -> `cclawcore-labs/easypanel-templates`
...
- [ ] **Add the secret** `MARKETPLACE_PAT` to the `cclawcore-labs/cclawcore` repo secrets

### 2. Install the Workflow

Copy `sync-marketplace-templates.yml` to `.github/workflows/` in the cclawcore repo.
...
2. Add `templates/compose/cclawcore.yaml` and `svgs/cclawcore.svg`
...
2. Add `blueprints/cclawcore/` with all 3 files
...
2. Add `templates/cclawcore/` with `meta.yaml`, `index.ts`, and `assets/logo.svg`
...
├── coolify/
│   └── cclawcore.yaml                   # -> coollabsio/coolify templates/compose/
├── dokploy/
│   ├── meta-entry.json                 # -> merge into Dokploy/templates meta.json
│   └── blueprints/cclawcore/
│       ├── docker-compose.yml          # -> Dokploy/templates blueprints/cclawcore/
│       └── template.toml              # -> Dokploy/templates blueprints/cclawcore/
└── easypanel/
    ├── meta.yaml                       # -> easypanel-io/templates templates/cclawcore/
    ├── index.ts                        # -> easypanel-io/templates templates/cclawcore/
    └── assets/                         # -> easypanel-io/templates templates/cclawcore/assets/
        └── (logo.svg goes here)
```
