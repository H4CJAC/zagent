...
Complete guide to setting up a CclawCore-powered robot on Raspberry Pi 5.
...
mkdir -p ~/.cclawcore/models
cp models/ggml-base.bin ~/.cclawcore/models/
```
...
mkdir -p ~/.cclawcore/models/piper
cd ~/.cclawcore/models/piper
wget https://huggingface.co/rhasspy/piper-voices/resolve/main/en/en_US/lessac/medium/en_US-lessac-medium.onnx
wget https://huggingface.co/rhasspy/piper-voices/resolve/main/en/en_US/lessac/medium/en_US-lessac-medium.onnx.json

# Test
echo "Hello, I am your robot!" | piper --model ~/.cclawcore/models/piper/en_US-lessac-medium.onnx --output_file test.wav
aplay test.wav
```
...
### 7. Build CclawCore Robot Kit

```bash
# Clone repo (or copy from USB)
git clone https://github.com/cclawcore-labs/cclawcore
cd cclawcore

# Build robot kit
cargo build --release -p cclawcore-robot-kit

# Build main cclawcore (optional, if using as agent)
cargo build --release
```
...
```bash
mkdir -p ~/.cclawcore
nano ~/.cclawcore/robot.toml
```

```toml
# ~/.cclawcore/robot.toml - Real Hardware Configuration
...
# Test speaker
echo "Testing speaker" | piper --model ~/.cclawcore/models/piper/en_US-lessac-medium.onnx --output_file - | aplay -D plughw:0,0
...
FIFO_PATH = "/tmp/cclawcore_sensors.fifo"
...
### Start CclawCore Agent

```bash
# Configure CclawCore to use robot tools
cat > ~/.cclawcore/config.toml << 'EOF'
...
cp ~/cclawcore/crates/robot-kit/SOUL.md ~/.cclawcore/workspace/

# Start agent
./target/release/cclawcore agent
```
...
# Start sensor loop
if [ ! -p /tmp/cclawcore_sensors.fifo ]; then
    mkfifo /tmp/cclawcore_sensors.fifo
fi
...
# Start cclawcore
cd ~/cclawcore
./target/release/cclawcore daemon &
AGENT_PID=$!
...
```bash
# /etc/systemd/system/cclawcore-robot.service
sudo tee /etc/systemd/system/cclawcore-robot.service << 'EOF'
[Unit]
Description=CclawCore Robot
After=network.target ollama.service

[Service]
Type=simple
User=pi
WorkingDirectory=/home/pi/cclawcore
...
sudo systemctl enable cclawcore-robot
sudo systemctl start cclawcore-robot

# Check status
sudo systemctl status cclawcore-robot
journalctl -u cclawcore-robot -f  # View logs
```
