...
A complete toolkit for building AI-powered robots with CclawCore. Designed for Raspberry Pi deployment with offline Ollama inference.
...
### 1. Build CclawCore with robot tools

```bash
# Clone and build
git clone https://github.com/cclawcore-labs/cclawcore
cd cclawcore
cargo build -p cclawcore-robot-kit --release
```
...
# Copy config
mkdir -p ~/.cclawcore
cp crates/robot-kit/robot.toml ~/.cclawcore/
cp crates/robot-kit/SOUL.md ~/.cclawcore/workspace/
...
# Test in mock mode
./target/release/cclawcore agent -m "Say hello and show a happy face"

# Test with real hardware
# (after configuring robot.toml)
./target/release/cclawcore agent -m "Move forward 1 meter"
```
...
```rust
use cclawcore_robot_kit::{create_tools, RobotConfig};
...
If you want runtime registration in `cclawcore`, add a thin adapter that maps this
...
mkdir cclawcore-robot-kit
cp -r target/release/cclawcore cclawcore-robot-kit/
cp -r examples/robot_kit cclawcore-robot-kit/
cp -r ~/.cclawcore cclawcore-robot-kit/dot-cclawcore

# Include models
mkdir -p cclawcore-robot-kit/models
cp ~/.cclawcore/models/ggml-base.bin cclawcore-robot-kit/models/
# Note: Ollama models are large, may want to download on target

# Create tarball
tar -czvf cclawcore-robot-kit.tar.gz cclawcore-robot-kit/

# Copy to USB
cp cclawcore-robot-kit.tar.gz /media/usb/TarBalls/
```
