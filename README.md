...
<h1 align="center">🦀 CclawCore — Personal AI Assistant</h1>
...
> **Preferred setup:** run `cclawcore onboard` in your terminal. CclawCore Onboard guides you step by step through setting up the gateway, workspace, channels, and provider. It is the recommended setup path and works on macOS, Linux, and Windows (via WSL2). New install? Start here: [Getting started](#quick-start)
...
`cclawcore onboard` runs automatically after install to configure your workspace and provider.
...
Upgrading? Run `cclawcore doctor` after updating.
...
CclawCore can import your OpenClaw workspace, memory, and configuration:
...
This migrates your memory entries, workspace files, and configuration from `~/.openclaw/` to `~/.cclawcore/`. Config is converted from JSON to TOML automatically.

## Security defaults (DM access)

CclawCore connects to real messaging surfaces. Treat inbound DMs as untrusted input.
...
- Approve with: `cclawcore pairing approve <channel> <code>` (then the sender is added to a local allowlist).
- Public inbound DMs require an explicit opt-in in `config.toml`.
- Run `cclawcore doctor` to surface risky or misconfigured DM policies.
...
| ---------- | ----------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-02-19 | _Critical_  | We are **not affiliated** with `openagen/cclawcore`, `cclawcore.org` or `cclawcore.net`. The `cclawcore.org` and `cclawcore.net` domains currently points to the `openagen/cclawcore` fork, and that domain/repository are impersonating our official website/project.                                                                                       | Do not trust information, binaries, fundraising, or announcements from those sources. Use only [this repository](https://github.com/cclawcore-labs/cclawcore) and our verified social accounts.                                                                                                                                                                                                                                                                                                                                                                                                                       |
| 2026-02-21 | _Important_ | Our official website is now live: [cclawcorelabs.ai](https://cclawcorelabs.ai). Thanks for your patience while we prepared the launch. We are still seeing impersonation attempts, so do **not** join any investment or fundraising activity claiming the CclawCore name unless it is published through our official channels.                            | Use [this repository](https://github.com/cclawcore-labs/cclawcore) as the single source of truth. Follow [X (@cclawcorelabs)](https://x.com/cclawcorelabs?s=21), [Facebook (Group)](https://www.facebook.com/groups/cclawcorelabs), and [Reddit (r/cclawcorelabs)](https://www.reddit.com/r/cclawcorelabs/) for official updates. |
...
> Notes: CclawCore results are measured on release builds using `/usr/bin/time -l`. OpenClaw requires Node.js runtime (typically ~390MB additional memory overhead), while NanoBot requires Python runtime. PicoClaw and CclawCore are static binaries. The RAM figures above are runtime memory; build-time compilation requirements are higher.
...
CclawCore supports subscription-native auth profiles (multi-account, encrypted at rest).
...
Workspace root: `~/.cclawcore/workspace/` (configurable via config).
...
Skills: `~/.cclawcore/workspace/skills/<skill>/SKILL.md` or `SKILL.toml`.
...
Or skip the steps above and install everything (system deps, Rust, CclawCore) in a single command:
...
- Follow [CclawCore Onboard](#quick-start) for a guided setup.
...
CclawCore was built for the smooth crab 🦀, a fast and efficient AI assistant. Built by Argenis De La Rosa and the community.
...
If CclawCore helps your work and you want to support ongoing development, you can donate here:
...
New to CclawCore? Look for issues labeled [`good first issue`](https://github.com/cclawcore-labs/cclawcore/issues?q=is%3Aissue+is%3Aopen+label%3A%22good+first+issue%22) — see our [Contributing Guide](CONTRIBUTING.md#first-time-contributors) for how to get started. AI/vibe-coded PRs welcome! 🤖
...
- New `Skill` → `~/.cclawcore/workspace/skills/<name>/`
...
**This is the only official CclawCore repository:**

> https://github.com/cclawcore-labs/cclawcore

Any other repository, organization, domain, or package claiming to be "CclawCore" or implying affiliation with CclawCore Labs is **unauthorized and not affiliated with this project**. Known unauthorized forks will be listed in [TRADEMARK.md](docs/maintainers/trademark.md).

If you encounter impersonation or trademark misuse, please [open an issue](https://github.com/cclawcore-labs/cclawcore/issues).

---

## License

CclawCore is dual-licensed for maximum openness and contributor protection:
...
The **CclawCore** name and logo are trademarks of CclawCore Labs. This license does not grant permission to use them to imply endorsement or affiliation. See [TRADEMARK.md](docs/maintainers/trademark.md) for permitted and prohibited uses.
...
**CclawCore** — Zero overhead. Zero compromise. Deploy anywhere. Swap anything. 🦀
