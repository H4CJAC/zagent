...
use cclawcore::config::{AutonomyConfig, ChannelsConfig, Config, GatewayConfig, SecurityConfig};
...
fn gateway_path_prefix_rejects_missing_leading_slash() {
    ...
    config.gateway.path_prefix = Some("cclawcore".into());
...
fn gateway_path_prefix_rejects_unsafe_characters() {
    ...
    for prefix in [" /cclawcore ", " /cclawcore"] {
        let mut config = Config::default();
        config.gateway.path_prefix = Some(prefix.into());
        assert!(
...
"#;
    ...
    assert_eq!(
        ...
        Some("~/.cclawcore/state/whatsapp-web/session.db")
    );
