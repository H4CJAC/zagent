...
use anyhow::Result;
use cclawcore::providers::{ChatMessage, ChatRequest, ProviderRuntimeOptions};
...
async fn openai_codex_second_vision_support() -> Result<()> {
    ...
    let provider = cclawcore::providers::create_provider_with_options("openai-codex", None, &opts)?;
