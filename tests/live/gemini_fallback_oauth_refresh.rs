...
//!
//! Requires:
//! - Live Gemini OAuth profile in `~/.cclawcore/auth-profiles.json` with refresh_token
...
async fn gemini_warmup_refreshes_expired_oauth_token() -> Result<()> {
    // Find ~/.cclawcore/auth-profiles.json
    let home = env::var("HOME").expect("HOME env var not set");
    let cclawcore_dir = PathBuf::from(home).join(".cclawcore");
    let auth_profiles_path = cclawcore_dir.join("auth-profiles.json");
    ...
    if !auth_profiles_path.exists() {
        ...
        eprintln!("Run: cclawcore auth login --provider gemini");
        return Ok(());
    }
    ...
    let gemini_profile_key = profiles
        ...
        .ok_or_else(|| {
            anyhow::anyhow!(
                "No Gemini OAuth profile found. Run: cclawcore auth login --provider gemini"
            )
        })?
        .clone();
    ...
    // This will load auth from ~/.cclawcore/auth-profiles.json (with expired token)
    let provider = cclawcore::providers::create_provider("gemini", None)?;
