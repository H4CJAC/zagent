...
BIN_PATH="${1:-$ROOT_DIR/target/debug/cclawcore}"
TMP_ROOT="/tmp/cclawcore-tmux-onboard-$$"
...
if [[ ! -x "$BIN_PATH" ]]; then
  echo "Building cclawcore..."
  cargo build --bin cclawcore >/dev/null
fi
...
run_update_custom_model_flow() {
  ...
  env CCLAWCORE_CONFIG_DIR="$config_dir" \
