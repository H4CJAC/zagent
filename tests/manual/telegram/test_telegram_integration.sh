#!/bin/bash
# CclawCore Telegram Integration Test Suite
...
if [ -f "target/release/cclawcore" ]; then
    BINARY_SIZE=$(ls -lh target/release/cclawcore | awk '{print $5}')
    SIZE_BYTES=$(stat -f%z target/release/cclawcore 2>/dev/null || stat -c%s target/release/cclawcore)
...
CONFIG_PATH="$HOME/.cclawcore/config.toml"
if [ -f "$CONFIG_PATH" ]; then
    ...
    if grep -q "\[channels_config.telegram\]" "$CONFIG_PATH"; then
        ...
        warn "Telegram not configured - run 'cclawcore onboard' first"
    fi
else
    warn "No config file found - run 'cclawcore onboard' first"
fi
...
START_TIME=$(date +%s)
HEALTH_OUTPUT=$(timeout 10 target/release/cclawcore channel doctor 2>&1 || true)
END_TIME=$(date +%s)
