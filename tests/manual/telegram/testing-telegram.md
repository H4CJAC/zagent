...
This guide covers testing the Telegram channel integration for CclawCore.
...
6. **Error logging**
    ...
    ```bash
    RUST_LOG=debug cclawcore channel start
    ```
...
Solution: Check user allowlist
  ...
  4. Run: cclawcore onboard --channels-only
```
...
# 3. Monitor logs
RUST_LOG=info cclawcore daemon
...
| ---------------------- | ---------- | -------------------------------- |
| Health check time      | <5s        | `time cclawcore channel doctor`   |
| First response time    | <3s        | Time from sending to receiving   |
| Message split overhead | <50ms      | Check logs for timing            |
| Memory usage           | <10MB      | `ps aux \| grep cclawcore`        |
| Binary size            | ~3-4MB     | `ls -lh target/release/cclawcore` |
...
```bash
# Maximum logging
RUST_LOG=trace cclawcore channel start
...
# Validate config
cat ~/.cclawcore/config.toml | grep -A 3 "\[channels_config.telegram\]"
```
...
- [CclawCore Main README](../../README.md)
- [Contributing Guide](../../CONTRIBUTING.md)
- [Issue Tracker](https://github.com/cclawcore-labs/cclawcore/issues)
