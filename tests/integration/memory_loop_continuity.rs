...
use cclawcore::memory::sqlite::SqliteMemory;
use cclawcore::memory::traits::{Memory, MemoryCategory};
use cclawcore::providers::ToolCall;
...
async fn compressor_with_memory_saves_summary() {
    use cclawcore::agent::context_compressor::{ContextCompressionConfig, ContextCompressor};
    use cclawcore::providers::traits::ChatMessage;
...
async fn consolidation_extracts_facts_to_memory() {
    ...
    let result = cclawcore::memory::consolidation::consolidate_turn(
        &provider,
        "test-model",
        mem.as_ref(),
...
async fn memory_survives_rapid_consolidation() {
    ...
    for i in 0..10 {
        ...
        let _ = cclawcore::memory::consolidation::consolidate_turn(
            &provider,
            "test-model",
            mem.as_ref(),
...
async fn session_backend_persists_messages() {
    use cclawcore::channels::session_backend::SessionBackend;
    use cclawcore::channels::session_sqlite::SqliteSessionBackend;
    use cclawcore::providers::traits::ChatMessage;
