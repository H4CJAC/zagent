...
pub fn tooltip_for_state(connected: bool, status: AgentStatus) -> &'static str {
    if !connected {
        return "CclawCore — Disconnected";
    }
    match status {
        AgentStatus::Idle => "CclawCore — Idle",
        AgentStatus::Working => "CclawCore — Working",
        AgentStatus::Error => "CclawCore — Error",
    }
}
